...
Problem: Activity Monitor shows Helix at 450–600MB on viewer-only laptops and
>1GB after heavy use. Target: ~150–250MB steady-state, flat over a workday.

## 1. What we measured

Engine benchmarks ran the real engine in a debug build (v0.1.12, when it still had a
headless mode) with the
mock harness, offline, on Linux/glibc; RSS sampled from `/proc`. Scripts:
`/tmp/helix-mem-test/{run,bench2,bench3}.sh` (to be promoted to
...
|---|---|---|---|
| Allocator | mimalloc (or jemalloc) as global alloc in `apps/helix` | system malloc watermark; churn sources below | watermark becomes recoverable; biggest single lever on macOS |
...
Phases 1–2 landed on `helix/helix-memory-usage-investigation` (phase 3
