...
//!
//! Both viewports share this module (the `helix_proto::view` rule: derivations
...
fn try_text_append(prev: &SessionMessageEntry, next: &SessionMessageEntry) -> Option<TextAppend> {
  ...
  for (p, n) in prev.parts.iter().zip(&next.parts) {
    ...
    let (MessagePart::Text { id: pid, text: pt }, MessagePart::Text { id: nid, text: nt }) = (p, n)
    else {
      return None;
    };
...
pub fn apply_transcript_frame(
  ...
  match frame {
    ...
    TranscriptFrame::Delta {
      ...
      if !remove.is_empty() {
        let gone: std::collections::HashSet<&str> = remove.iter().map(String::as_str).collect();
