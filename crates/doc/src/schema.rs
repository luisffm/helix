...
impl SessionDoc {
  ...
  pub fn set_command_status(
    ...
    for i in 0..commands.len() {
      if let Some(loro::ValueOrContainer::Container(loro::Container::Map(map))) = commands.get(i) {
  ...
  pub fn set_message_status(
    ...
    for i in 0..messages.len() {
      if let Some(loro::ValueOrContainer::Container(loro::Container::Map(map))) = messages.get(i) {
        let id_matches = matches!(
            map.get("id"),
  ...
  pub fn append_error_part(
    ...
    let messages = self.doc.get_list("messages");
    for i in 0..messages.len() {
      let Some(loro::ValueOrContainer::Container(loro::Container::Map(entry))) = messages.get(i)
      else {
        continue;
      };
      let id_matches = matches!(
      ...
      for j in 0..parts.len() {
        if let Some(loro::ValueOrContainer::Container(loro::Container::Map(part))) = parts.get(j)
          && matches!(
              part.get("id"),
  ...
  pub fn resolve_input(&self, request_id: &str) -> Result<bool, DocError> {
    ...
    for i in 0..messages.len() {
      ...
      for j in 0..parts.len() {
        let Some(loro::ValueOrContainer::Container(loro::Container::Map(part))) = parts.get(j)
        else {
          continue;
        };
...
///
/// Port of helix's `DocSegmentWriter` diff discipline: called with the *folded* parts of the
  ...
  pub fn sync(&mut self, folded: &[MessagePart]) -> Result<(), DocError> {
    ...
    for (i, part) in folded.iter().enumerate() {
      match self.written.get(i) {
        ...
        Some(prev) => {
          match (prev, part) {
            (MessagePart::Text { text: old, .. }, MessagePart::Text { text: new, .. })
              if new.starts_with(old.as_str()) =>
            {
              ...
              if !delta.is_empty() {
                ...
                match part_map.get("text") {
                  ...
                  _ => {
                    return Err(DocError::Schema("text part missing LoroText".into()));
                  }
                }
                dirty = true;
              }
...
mod tests {
  ...
  use helix_proto::{AgentEvent, ToolCall};
  ...
  fn segment_writer_round_trips_stripped_tool_fields() {
    ...
    fold_event_into_parts(
      ...
      &AgentEvent::ToolResult {
        ...
        is_error: false,
        output: Some("total 0\nmore lines".into()),
        diff: Some(helix_proto::ToolDiff {
          path: "/w/a.rs".into(),
          old_text: Some("old\n".into()),
          new_text: "new\n".into(),
        }),
  ...
  fn set_message_status_stamps_existing_entry() {
    ...
    assert!(
      doc
        .set_message_status("m1", MessageStatus::Aborted)
        .unwrap()
    );
    assert!(
      !doc
        .set_message_status("nope", MessageStatus::Aborted)
        .unwrap()
    );
    let entries = doc.read_entries().unwrap();
  ...
  fn command_queue_and_outcome_round_trip() {
    ...
    doc
      .set_command_status("c1", SessionCommandStatus::Applied, None)
      .unwrap();
  ...
  fn tail_materializes_last_n_joined() {
    ...
    for i in 0..5 {
      doc
        .push_message(&user_entry(&format!("m{i}"), &format!("msg {i}")))
        .unwrap();
    }
