...
use helix_proto::{AgentEvent, ToolCall, ToolDiff, UserInputQuestion};
...
impl MessagePart {
  ...
  pub fn byte_len(&self) -> usize {
    match self {
      ...
      MessagePart::Input { questions, .. } => serde_json::to_vec(questions).map_or(0, |v| v.len()),
...
///
/// Semantics from helix `foldEventIntoParts`:
...
pub fn fold_event_into_parts(out: &mut Vec<MessagePart>, event: &AgentEvent) {
  match event {
    ...
    AgentEvent::ReasoningDelta { .. } => {
      // Reasoning is not rendered as a transcript part (matches helix).
    }
...
/// continuations (`continuation_id(root, i)`), matching `splitMessageEntry` in helix.
  ...
  fn session_started_resets_accumulator() {
    ...
    fold_event_into_parts(
      ...
      &AgentEvent::SessionStarted {
        harness: helix_proto::HarnessId::Mock,
        model: "m".into(),
        tools: vec![],
        cwd: "/".into(),
        session_id: "s".into(),
