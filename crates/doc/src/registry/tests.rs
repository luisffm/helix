...
use super::*;
use helix_proto::{HarnessId, SandboxLevel, SessionStatus};
...
fn rows_round_trip_and_upsert_refreshes() {
  ...
  doc
    .upsert_session(&session("chat-1", "dev-a", SessionStatus::Working))
    .unwrap();
...
fn late_create_chat_config_survives_a_prior_claim() {
  ...
  // The composer's createChat writes the real row first (older clocks)…
  doc.upsert_chat(&chat("chat-race", "dev-a")).unwrap();
  // …then the executor claims the same chat on the first run command. (The
  // sleep keeps the HLCs out of the same-millisecond tiebreak.)
  std::thread::sleep(std::time::Duration::from_millis(2));
  doc.claim_chat("chat-race", Some("/tmp/repo"), Some("space-1"), ts(9_000));
  ...
  // …and the fields the claim never wrote survive it.
...
#[test]
fn persistence_round_trips_rows_and_clock() {
  let mut doc = RegistryDoc::new("dev-a");
  doc.upsert_chat(&chat("chat-1", "dev-a")).unwrap();
  doc.rename_chat("chat-1", "renamed").unwrap();

  let bytes = doc.to_bytes().unwrap();
  let mut restored = RegistryDoc::from_bytes(&bytes, "dev-a").unwrap();
  assert_eq!(restored.read_all().unwrap(), doc.read_all().unwrap());
  assert_eq!(
    restored.chat("chat-1").unwrap().unwrap().title.as_deref(),
    Some("renamed")
  );
  // The restored clock keeps stamping ahead of the persisted rows.
  restored.rename_chat("chat-1", "after restart").unwrap();
  assert_eq!(
    restored.chat("chat-1").unwrap().unwrap().title.as_deref(),
    Some("after restart")
  );
...
#[test]
fn migration_seeds_rows_that_lose_to_live_writes() {
  ...
  let seeded = doc
    ...
    .unwrap();
  assert_eq!(seeded, 4);
  // The seeded rows serve the full sidebar immediately.
  let state = doc.read_all().unwrap();
  assert_eq!(state, legacy.read_all().unwrap());

  // A live rename (now-clock) beats the migrated title.
  doc.rename_chat("chat-1", "live rename").unwrap();
  assert_eq!(
