//! Workspace registry — the row-table sidebar store that replaces the Loro
//! workspace doc (docs/registry-sync.md).
//!
//! [`RegistryDoc`] holds the four sidebar tables (devices, spaces, chats,
//! sessions) as rows merged by [`apply_op`] under an HLC clock: every local
//! write is stamped and applied in place. The per-field LWW merge is kept
//! because rows carry per-field clocks and deletes are tombstones — a claim
//! racing a create must not lose the fields it never wrote.
...
/// The registry store. Pure data — no I/O, no async; the engine host drives it
/// under a lock.
pub struct RegistryDoc {
  device_id: String,
  /// kind → id → row.
  rows: HashMap<String, HashMap<String, RegistryRow>>,
  clock: HlcClock,
  /// Bumped on every mutation — the engine host converts this into
  /// watch-channel publishes and snapshot debounces.
  generation: u64,
}
...
impl RegistryDoc {
  ...
  fn put_row(&mut self, row: RegistryRow) {
    self
      .rows
  ...
  /// Apply local ops in place. Nothing acknowledges a write here, so the
  /// stamped op IS the row's truth the moment it is made.
  fn apply_local_ops(&mut self, ops: Vec<RowOp>) {
    if ops.is_empty() {
      return;
    }
    self.generation += 1;
    for op in &ops {
      let current = self.rows.get(&op.kind).and_then(|m| m.get(&op.id));
      if let (Some(next), _) = apply_op(current, op) {
        self.put_row(next);
      }
    }
  }
  ...
  fn write(&mut self, kind: &str, id: &str, op: OpKind, set: BTreeMap<String, Value>) {
    ...
    self.apply_local_ops(vec![RowOp {
      kind: kind.to_string(),
      id: id.to_string(),
      op,
      set: Some(set),
  ...
  /// The live row, or `None` when it is missing or tombstoned.
  fn live_row(&self, kind: &str, id: &str) -> Option<RegistryRow> {
    self
      .rows
      .get(kind)
      .and_then(|m| m.get(id))
  ...
  /// Every live row of `kind`.
  fn live_rows(&self, kind: &str) -> Vec<RegistryRow> {
    self
      .rows
      .get(kind)
      .map(|m| m.values().filter(|row| !row.deleted).cloned().collect())
      .unwrap_or_default()
  }

  fn read_kind<T: serde::de::DeserializeOwned>(&self, kind: &str) -> Vec<T> {
    let mut out = Vec::new();
    for row in self.live_rows(kind) {
  ...
  pub fn set_chat_seen(&mut self, chat_id: &str, at: DateTime<Utc>) -> Result<bool, DocError> {
    let Some(row) = self.live_row(KIND_CHATS, chat_id) else {
      return Ok(false);
    };
  ...
  pub fn seed_from_workspace(&mut self, state: &WorkspaceState) -> Result<usize, DocError> {
    ...
    for chunk in ops.chunks(400) {
      self.apply_local_ops(chunk.to_vec());
    }
    Ok(count)
  }
}
...
// ("idle"/"working"/…) — helix_proto's serde derives already use camelCase;
