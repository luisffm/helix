//! Workspace doc schema over `loro` — the per-org entity index that replaces helix's
...
//! sets from any device — matching helix's Mutate surface. Presence rides the room's
//! `EphemeralStore` under keys `presence/{deviceId}` (an online timestamp), replacing
//! helix's 15s heartbeat writes so liveness never grows the oplog.
//!
//! Timestamps are stored as epoch millis (the session-doc convention) and surface as
//! `chrono::DateTime<Utc>` through the `helix_proto` entity types.

use chrono::{DateTime, Utc};
use loro::{ExportMode, LoroDoc, LoroMap, LoroValue, ToJson};
use serde::{Deserialize, Serialize};

use helix_proto::{Chat, ChatConfig, Device, Session, SessionStatus, Space};
...
impl WorkspaceDoc {
  ...
  pub fn set_device_last_seen(&self, device_id: &str, at: DateTime<Utc>) -> Result<bool, DocError> {
    let Some(row) = self.existing_row("devices", device_id) else {
      return Ok(false);
    };
  ...
  /// latest run and the cwd it was created under (helix stored the same pair
  ...
  fn read_rows<T: serde::de::DeserializeOwned>(&self, container: &str) -> Result<Vec<T>, DocError> {
...
mod tests {
  use super::*;
  use helix_proto::{HarnessId, SandboxLevel};
