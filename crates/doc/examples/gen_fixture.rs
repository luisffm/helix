...
//! (`edge/scripts/compat-check.mjs`). Usage: `cargo run -p helix-doc --example gen_fixture -- <out>`
...
fn main() {
  ...
  let mut writer =
    SegmentWriter::begin(&doc, "m-assistant-1", "device-rust", 1_700_000_001_000).expect("begin");
  ...
  doc
    .set_command_status("cmd-1", SessionCommandStatus::Applied, None)
    .expect("outcome");
