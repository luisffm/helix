...
use helix_proto::{GitHistoryRefKind, TerminalEvent};
...
async fn repos_round_trip_add_branches_worktrees() {
  ...
  // Worktree add: helix/<name> branch, isolated dir under the test root.
  let worktree = repos
    .create_worktree(&repo_dir, "main")
    .await
    .expect("worktree");
  assert!(
    worktree.branch.starts_with("helix/"),
    "branch: {}",
    worktree.branch
  );
  ...
  // worktree's helix/<name> branch maps to its linked-checkout path, and
  ...
  // Delete: dir removed, helix branch removed, refs pruned.
  repos
...
async fn git_history_is_topological_paged_and_carries_public_refs() {
  ...
  assert!(
    first.commits[0]
      .refs
      .iter()
      .any(|reference| { reference.kind == GitHistoryRefKind::Tag && reference.label == "v1" })
  );
...
async fn diff_file_text_returns_both_checked_sources() {
  ...
  let escape = helix_proto::DiffFileSummary {
...
async fn diff_capture_truncates_at_patch_cap() {
  ...
  assert!(snapshot.patch.contains("# Helix diff truncated"));
}
...
async fn rpc_dispatch_for_m5_methods() {
  ...
  unsafe { std::env::set_var("HELIX_WORKTREES_DIR", tmp.path().join("worktrees")) };
  let core = assemble(&tmp.path().join("data"));
  let client = helix_rpc::memory_client(core.rpc_service());
