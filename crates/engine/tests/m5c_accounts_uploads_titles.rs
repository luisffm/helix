...
use helix_harness::mock::MockHarness;
use helix_proto::{AgentAccountsSnapshot, AgentEvent, DoneStatus, HarnessId, SandboxLevel};
...
async fn uploads_chunk_commit_readback_and_jail() {
  ...
  // Non-image extensions are refused even inside the jail (helix parity).
...
async fn titling_e2e_names_chat_and_renames_worktree_branch() {
  ...
  let request = helix_proto::RunRequest {
  ...
  assert_eq!(chat.branch.as_deref(), Some("helix/fix-login-flow"));
...
async fn rename_worktree_branch_guards_and_collisions() {
  ...
  let unchanged = repos
    .rename_worktree_branch(wt_path, "helix/not-this-one", "Some Title")
    .await
    .expect("guarded");
  ...
  let renamed = repos
    ...
    .await
    .expect("renamed");
  assert_eq!(renamed, "helix/add-dark-mode");

  // Already renamed → the guard (branch no longer helix/<folder>) makes any
  // further title rename a no-op.
  let again = repos
    .rename_worktree_branch(wt_path, "helix/add-dark-mode", "Different Title")
    .await
    .expect("second rename");
  assert_eq!(again, "helix/add-dark-mode");
  ...
  assert!(
    renamed2.starts_with("helix/add-dark-mode-")
      && renamed2.len() == "helix/add-dark-mode-".len() + 6,
    "suffixed: {renamed2}"
  );
  ...
  assert_eq!(worktree_branch_from_title("***"), "helix/update");
  assert_eq!(
    worktree_branch_from_title("Cafe's Dark Mode"),
    "helix/cafes-dark-mode"
  );
}
...
async fn rpc_dispatch_for_m5c_methods() {
  ...
  let client = helix_rpc::memory_client(core.rpc_service());
