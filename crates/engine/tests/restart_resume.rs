...
//!   session id as `RunRequest.resume` (engine-owned, helix sessions.ts:736);
//! - a kill -9 style crash recovers the session id from the run journal
//!   (helix recoverDraft, sessions.ts:538-552) and stamps streaming entries
//!   `aborted`;
...
use helix_engine::{EngineCore, HarnessRegistry, RunJournal};
use helix_harness::{Harness, HarnessError, RunControls};
use helix_proto::{
  AgentEvent, DoneStatus, HarnessId, Model, ReasoningLevel, RunRequest, SandboxLevel, SteeringMode,
};
...
async fn persistent_session_serves_multiple_turns_on_one_child() {
  ...
  // The session PARKS (helix runsBySession): the second message routes into
...
async fn fresh_crash_auto_resumes_and_notes_the_interruption() {
  ...
  // The run is PICKED BACK UP without any user action (helix: "not just
...
/// Run with: `cargo test -p helix-engine --test restart_resume -- --ignored`
...
async fn real_claude_remembers_codeword_across_engine_restart() {
  ...
  let assemble_real = || {
    EngineCore::assemble(
      &dir,
      Arc::new(helix_engine::default_registry()),
  ...
  core
    ...
    .queue_command(
      CHAT,
      SessionCommandPayload::Run {
        request: real_request("Remember the codeword: PINEAPPLE. Reply with exactly: stored"),
