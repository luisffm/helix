...
//!    existed in the agent's session data, was absent from helix's doc).
...
use helix_engine::{EngineCore, HarnessRegistry, SteerOutcome};
...
fn init_quiesce_env() {
  ...
  ONCE.call_once(|| {
    ...
    unsafe { std::env::set_var("HELIX_TURN_QUIESCE_MS", QUIESCE_MS.to_string()) };
  });
}
...
async fn missing_turn_end_settles_instead_of_working_forever() {
  ...
  assert!(
    texts
      .iter()
      .any(|(t, s)| { t.contains("here are the results") && *s == Some(MessageStatus::Complete) }),
