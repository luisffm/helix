...
//! helix's `terminals.ts` over `portable-pty`).
//!
...
//!   owner, so the per-user owner re-checks from helix's Router land with real
...
use helix_doc::TERMINAL_OUTPUT_BATCH_MS;
