...
//! untitled chat, name it with the harness's cheapest model (port of helix's
...
//!    last listed model — helix's `cheapestModel`);
...
//!    retrying on helix's short backoff ladder; fall back to the prompt's first
//!    words when every attempt produces nothing;
//! 4. re-check the title (a user rename during generation wins);
//! 5. when the chat sits in a helix worktree (`helix/<name>` branch), rename the
...
use helix_harness::{CancellationToken, RunControls, SteerMessage};
use helix_proto::{
...
/// couple of times with a short backoff before falling back (helix's ladder).
...
impl TitleGenerator {
  ...
  async fn generate(
    ...
    // helix/<name> branch (guards live inside rename_worktree_branch).
    if let (Some(chat_cwd), Some(branch)) = (&latest.cwd, &latest.branch)
      && branch.starts_with("helix/")
    {
      match self
        .inner
        .repos
...
/// The cheapest model a harness offers (helix's `cheapestModel` heuristic):
...
async fn collect_text(
  harness: &dyn helix_harness::Harness,
  request: RunRequest,
