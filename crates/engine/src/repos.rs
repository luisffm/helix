...
//! browser (feature-inventory §3.5; port of helix's `repos.ts` + `folder-lister.ts`).
//!
...
//! created under `~/.helix/worktrees/<repoName>/<worktreeName>` (NOT the data
//! dir — worktrees are user-facing working checkouts), with an auto-generated name +
//! matching `helix/<name>` branch. `HELIX_WORKTREES_DIR` overrides the root.
//!
...
const NOUNS: &[&str] = &[
  "otter", "harbor", "falcon", "cedar", "meadow", "helix", "delta", "ember", "lynx", "maple",
...
/// worktrees are user-facing working checkouts. `HELIX_WORKTREES_DIR` overrides
/// (test isolation); empty reads as unset.
fn default_worktrees_root() -> PathBuf {
  std::env::var_os("HELIX_WORKTREES_DIR")
    .filter(|s| !s.is_empty())
    .map(PathBuf::from)
    .unwrap_or_else(|| home_dir().join(".helix").join("worktrees"))
}
...
impl Repos {
  ...
  /// comes from `$HELIX_WORKTREES_DIR` or `~/.helix/worktrees`.
  ...
  pub async fn fetch_all(&self, repo_path: &Path) -> Result<(), EngineError> {
    self
      .git(&["fetch", "--all", "--quiet"], Some(repo_path))
      .await
      .map(drop)
  }
  ...
  pub async fn clone_repo(&self, url: &str) -> Result<Repo, EngineError> {
    ...
    self
      .git(&["clone", trimmed, &target.to_string_lossy()], None)
      .await?;
  ...
  pub async fn refs(&self, repo_path: &Path) -> Result<Vec<RepoRef>, EngineError> {
    ...
    let mut worktrees: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    if let Ok(out) = self
  ...
  /// remotes, and tags seed the walk, so Helix's internal refs never leak
  ...
  pub async fn history(
    ...
    let head_commit_count = if cursor == 0 && head_sha.is_some() {
      self
        .git(&["rev-list", "--count", "HEAD"], Some(repo_path))
        .await
        .ok()
        .and_then(|count| count.parse().ok())
    } else {
  ...
  pub async fn switch_ref(&self, cwd: &Path, ref_name: &str) -> Result<String, EngineError> {
    ...
    if local {
      ...
      if has_remote {
        self
          .git(&["checkout", "--track", &remote], Some(cwd))
          .await?;
      } else {
  ...
  /// `{worktrees_root}/<repoName>/<generatedName>`, on a fresh `helix/<name>`
  /// branch off `branch`.
  pub async fn create_worktree(
    ...
    for attempt in 0..50u64 {
      ...
      if !base.join(&candidate).exists() && !existing.contains(&format!("helix/{candidate}")) {
        name = Some(candidate);
        break;
      }
    }
    ...
    let branch_name = format!("helix/{name}");
    self
      .git(
        &[
          "worktree",
  ...
  /// Rename a helix-created worktree branch after its chat's generated title
  /// (port of helix's `renameWorktreeBranch`). Guards:
  /// - respect an external checkout/rename: only act while the worktree is still
  ///   on `expected_branch` AND that branch is the original `helix/<folderName>`;
  ...
  pub async fn rename_worktree_branch(
    ...
    let folder = worktree_path
      ...
      .map(|n| n.to_string_lossy().to_string())
      .unwrap_or_default();
    if current != expected_branch || expected_branch != format!("helix/{folder}") {
      return Ok(current);
    }
  ...
  /// Deletes the worktree's branch ONLY when helix created it (`helix/…`) — the
  ...
  pub async fn list_folders(&self, path: Option<String>) -> Result<FolderListing, EngineError> {
    self
      .list_folders_with(path, FOLDER_LIST_TIMEOUT, false)
      .await
  }
  ...
  /// timeout the thread is simply abandoned (the helix backend's disposable
...
fn list_folders_blocking(target: &Path) -> Result<FolderListing, EngineError> {
  let read = std::fs::read_dir(target).map_err(|e| match e.kind() {
    std::io::ErrorKind::PermissionDenied => {
      EngineError::Other("Helix doesn't have access to this folder on the device.".into())
    }
...
/// Turn a generated chat title into the semantic portion of a Helix branch
/// (port of helix's `worktreeBranchFromTitle`). Helix NFKD-normalizes accented
...
pub fn worktree_branch_from_title(title: &str) -> String {
  ...
  format!("helix/{}", if slug.is_empty() { "update" } else { slug })
}
...
mod tests {
  ...
  fn cancelled_search_stops_before_walking() {
    ...
    let err =
      search_files_blocking_with_cancel(root.path(), "", &[], || cancelled.load(Ordering::Relaxed))
        .unwrap_err()
        .to_string();
