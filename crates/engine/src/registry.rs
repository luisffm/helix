...
use helix_harness::{Harness, HarnessError, mock::MockHarness};
use helix_proto::{AgentEvent, DoneStatus, HarnessId, ReasoningLevel, SteeringMode};
...
/// `claude-code` slot resolved through `helix_harness` on first use (subprocess
...
pub fn default_registry() -> HarnessRegistry {
  ...
  helix_harness::shell_env::prewarm();
  ...
  registry.register(Arc::new(MockHarness {
        script: vec![
            ...
            AgentEvent::ToolCall {
                id: "mock-tool-1".into(),
                call: helix_proto::ToolCall::Exec {
  ...
  registry.register_lazy(
    ...
    Box::new(|| helix_harness::AcpHarness::claude().installed()),
    Box::new(|| Ok(Arc::new(helix_harness::AcpHarness::claude()) as Arc<dyn Harness>)),
  );
  ...
  // helix_harness::codex::catalog. CLI discovery only happens when a
  ...
  registry.register_lazy(
    ...
    Box::new(|| helix_harness::AcpHarness::codex().installed()),
    Box::new(|| Ok(Arc::new(helix_harness::AcpHarness::codex()) as Arc<dyn Harness>)),
  );
  ...
  registry.register_lazy(
    ...
    Box::new(|| helix_harness::AcpHarness::cursor().installed()),
    Box::new(|| Ok(Arc::new(helix_harness::AcpHarness::cursor()) as Arc<dyn Harness>)),
  );
  ...
  registry.register_lazy(
    ...
    Box::new(|| helix_harness::AcpHarness::grok().installed()),
    Box::new(|| Ok(Arc::new(helix_harness::AcpHarness::grok()) as Arc<dyn Harness>)),
  );
  ...
  registry.register_lazy(
    ...
    Box::new(|| helix_harness::AcpHarness::hermes().installed()),
    Box::new(|| Ok(Arc::new(helix_harness::AcpHarness::hermes()) as Arc<dyn Harness>)),
  );
  ...
  registry.register_lazy(
    ...
    Box::new(|| helix_harness::AcpHarness::pi().installed()),
    Box::new(|| Ok(Arc::new(helix_harness::AcpHarness::pi()) as Arc<dyn Harness>)),
  );
  registry
}
