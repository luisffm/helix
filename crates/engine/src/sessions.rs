...
//!
//! Pragmatic port of helix's `sessions.ts` (spec: feature-inventory §3.2):
...
//!
//! Scope notes: sessions are keyed by chat id (one live run per chat). Helix's pulse
...
use helix_harness::{CancellationToken, Harness, RunControls, SteerMessage};
use helix_proto::{
...
/// directory — helix sessions.ts:563 "harness session stores are keyed by
...
struct Inner {
  ...
  /// (helix kept the same pair on `chats.harness_session_id`). An empty
...
impl SessionsEngine {
  pub fn new(device_id: String, journal: Arc<RunJournal>, registry: Arc<HarnessRegistry>) -> Self {
  ...
  pub fn any_active(&self) -> bool {
    lock(&self.inner.statuses).values().any(|s| {
      matches!(
        s.status,
        helix_proto::SessionStatus::Working | helix_proto::SessionStatus::AwaitingInput
      )
    })
  }
  ...
  ///   (helix's persistent-session routing); otherwise any live run is interrupted
  ...
  pub async fn dispatch(
    ...
    self
      .dispatch_with(chat_id, harness_id, request, message_id, false)
      .await
  }
  ...
  async fn dispatch_inner(
    ...
    // Engine-owned resume (helix sessions.ts:736 — every dispatch read the
  ...
  pub async fn interrupt(&self, chat_id: &str) -> Result<bool, EngineError> {
    ...
    // Unpark any blocked question FIRST (mirrors helix: harness teardown can await a
  ...
  /// the remembered harness session (helix: "not just eulogized";
  ...
  pub fn recover_stale(&self) -> Result<usize, EngineError> {
    ...
    for chat_id in stale {
      ...
      // same harness conversation (helix recoverDraft, sessions.ts:538).
      ...
      tokio::spawn(async move {
        ...
        let request = sessions
                    ...
                    // Last resort: the journal's own cwd (helix's draft config)
...
impl Inner {
  ...
  /// engine restart (helix sessions.ts:1039).
  ...
  /// (helix sessions.ts:736, looked up on every dispatch):
  ...
  fn resume_for(&self, chat_id: &str, cwd: &str) -> Option<String> {
    ...
    if let Some(known) = lock(&self.harness_sessions).get(chat_id).cloned() {
      return (!known.session_id.is_empty() && cwd_ok(&known.cwd)).then_some(known.session_id);
    }
    if let Some(ws) = self.workspace()
...
async fn drive_run(
  ...
  // PERSISTENT SESSION (helix runsBySession): a completed turn on a
  ...
  // a session nobody comes back to (helix SESSION_IDLE_MS).
  ...
  // moment output flows again, and nothing is lost. `HELIX_TURN_QUIESCE_MS`
  // overrides the window; 0 disables.
  let quiesce_after: Option<std::time::Duration> = match std::env::var("HELIX_TURN_QUIESCE_MS")
    .ok()
  ...
  // push the window during real thinking. `HELIX_SELF_TURN_QUIESCE_MS`
  // overrides; 0 falls back to the normal window. An explicit
  // `HELIX_TURN_QUIESCE_MS=0` still disables the watchdog entirely.
  let self_quiesce_after: Option<std::time::Duration> =
    match std::env::var("HELIX_SELF_TURN_QUIESCE_MS")
      .ok()
      .and_then(|v| v.parse::<u64>().ok())
    {
      Some(0) => None,
  ...
  let final_status = loop {
    let event: AgentEvent = tokio::select! {
        ...
        // Idle reaper (helix SESSION_IDLE_MS): a parked persistent session
        ...
        _ = tokio::time::sleep_until({
            ...
            && !folded.iter().any(|p| match p {
                MessagePart::Tool { id, resolved: false, .. } => {
                    id != helix_proto::LIVE_PLAN_TOOL_ID
                }
      ...
      let self_continued = idle_since.is_some_and(|parked_at| parked_at.elapsed() >= RESUME_GATE)
        && (matches!(
            ...
            AgentEvent::ToolCall { id, .. }
                if id == helix_proto::LIVE_PLAN_TOOL_ID || !seen_tools.contains(id)
        ));
      if self_continued {
        tracing::info!(
            chat = %chat_id,
    ...
    match &event {
      ...
      AgentEvent::ToolCall { id, .. } if id == helix_proto::LIVE_PLAN_TOOL_ID => {}
      AgentEvent::ToolResult { id, .. } if id == helix_proto::LIVE_PLAN_TOOL_ID => {}
      AgentEvent::ToolCall { id, .. } => {
    ...
    // Defensive rule from helix: a mid-run SessionStarted re-emission (Claude SDK
    ...
    if !skip_fold {
      ...
      // journal. To reintroduce: `helix_doc::sidecar_payload(&event)`
    ...
    if !folded.is_empty() && !dirty {
      dirty = true;
      flush_at = tokio::time::Instant::now() + std::time::Duration::from_millis(STREAM_COMMIT_MS);
    }
  };
