...
use helix_doc::{MessagePart, SessionCommandPayload};
use helix_proto::{ChatConfig, EngineInfo, HarnessId, ToolCall};
use helix_rpc::{RpcError, RpcReply, RpcService, methods, parse_params};
...
enum MutateParams {
  ...
  /// Full-config replace on the chat row (helix `SetChatConfig`): the
  ...
  async fn file_search_root(&self, p: &FileSearchParams) -> Result<std::path::PathBuf, RpcError> {
    ...
    match (&p.chat_id, &p.space_id) {
      ...
      (None, Some(space_id)) => {
        ...
        if let Some(requested) = self.repos.workspace_checkout(&space_path, &requested).await {
          Ok(requested)
        } else {
...
/// The transcript watch as delta frames (`helix_doc::transcript_delta`): a
...
fn doc_messages_stream(
  rx: watch::Receiver<Vec<helix_doc::SessionMessageEntry>>,
) -> BoxStream<'static, serde_json::Value> {
  use helix_doc::transcript_delta::{TranscriptFrame, diff_transcript};
  futures::stream::unfold(
    (rx, None::<Vec<helix_doc::SessionMessageEntry>>),
...
impl RpcService for EngineRpc {
  async fn handle(&self, method: &str, params: serde_json::Value) -> Result<RpcReply, RpcError> {
    match method {
      ...
      methods::WATCH_CHATS => Ok(RpcReply::Stream(watch_stream(self.workspace.watch_chats()))),
      ...
      methods::WATCH_SESSIONS => {
        // Local live statuses merged with the registry's rows.
        let merged = self
          .workspace
      ...
      methods::GET_CHECKOUT_DIFF => {
        ...
        Box::pin(async move {
          ...
          .map_err(|e| RpcError::Failed(e.to_string()))?;
          RpcReply::value(&helix_proto::CheckoutDiff {
      ...
      methods::GET_CHECKOUT_FILE_DIFF_TEXT => {
        ...
        Box::pin(async move {
          let p: helix_proto::GetCheckoutFileDiffTextRequest = parse_params(params)?;
          let identity = Box::pin(self.repos.checkout_identity(std::path::Path::new(&p.cwd)))
            .await
          ...
          let stale = || helix_proto::CheckoutFileDiffText {
          ...
          RpcReply::value(&helix_proto::CheckoutFileDiffText {
