//! Per-session on-disk event journal (port of helix's `run-journal.ts`, JSONL-shaped).
//!
...
impl RunJournal {
  ...
  /// Auto-resume revival budget (helix `resumeAttempt`/`MAX_AUTO_RESUME`):
  ...
  /// from a previous era (file replaced) — falls back to a full replay, mirroring helix.
  pub fn replay(
    &self,
    chat_id: &str,
    after_seq: u64,
