...
//! (feature-inventory §3.7 "Agent accounts"; port of helix's `agent-accounts.ts`).
//!
...
//! (`/usage` in Claude Code, `/status` in Codex). Unlike helix (fetch on every
...
/// One saved login (`{slotId}.json`), same field surface as helix's slot files.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct Slot {
  id: String,
...
impl LoginFlow {
  fn started_at(&self) -> Instant {
    match self {
      LoginFlow::Claude { started_at, .. } | LoginFlow::Codex { started_at, .. } => *started_at,
    }
  }
}
...
impl AgentAccounts {
  ...
  /// Lazy TTL sweep (helix uses a background fiber; native reaps on the next
