//! helix-harness — one interface over coding agents, all driven through the
...
pub struct RunControls {
  /// The run sends questions and awaits answers (blocks the agent, mirrors helix).
  pub request_input:
...
/// the proper background-crash message old helix showed (user requirement).
