...
fn pi_spec() -> AcpAgentSpec {
  AcpAgentSpec {
    ...
    steering_mode: SteeringMode::TurnBoundary,
    // pi's thinking ladder (minimal→max; its extra "off" tier has no helix
    ...
    if let Some(stderr) = child.stderr.take() {
      ...
      tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
          tracing::debug!(target: "helix_harness::acp", "stderr: {line}");
          tail.push(&line);
        }
      });
    }
...
/// Map an advertised `thought_level` value id onto helix's ladder.
...
fn models_from_session(session_response: &Value, catalog: &[Model]) -> Vec<Model> {
  ...
  let build =
    |id: &str, name: Option<&str>, description: Option<&str>, options: Vec<ModelOption>| -> Model {
      let exact = exact(id);
...
/// helix's own (forced to the no-prompts choice), model rides the model rows,
...
/// catalogs (helix never declares the boolean config capability, so adapters
...
fn initialize_params(harness: HarnessId) -> Value {
  ...
  json!({
      ...
      // is what helix wants — the working tree is the source of truth for
...
/// bypassPermissions and the codex harness's approvalPolicy "never" (helix
...
fn handle_server_request(
  ...
  match method {
    ...
    // finished. Image generation has nowhere to land in a helix session.
    CURSOR_TASK => {
    ...
    CURSOR_GENERATE_IMAGE => {
      client.respond(
                &id,
                json!({ "outcome": { "outcome": "rejected", "reason": "helix cannot render generated images" } }),
            );
      Vec::new()
    }
    _ => {
      tracing::debug!(target: "helix_harness::acp", "unhandled server request: {method}");
...
/// one of these gets a response even when helix has nothing to do with it.
...
/// One `cursor/ask_question` entry: the helix-side question plus what is
/// needed to answer it — the wire id, and the label→optionId table (helix's
  ...
  params
    ...
    .filter_map(|q| {
      ...
      let choices: Vec<(String, String)> = q
        ...
        .collect();
      // An option-less question has no answer helix could send back.
      if choices.is_empty() {
        return None;
      }
      Some(CursorQuestion {
        wire_id,
        question: UserInputQuestion {
          // Helix-minted: cursor's ids ("q1") repeat across turns.
          id: new_message_id(),
...
/// unblocks without helix inventing a pick.
  ...
  let picked: Vec<Value> = asked
    ...
    .filter_map(|asked| {
      ...
      let ids: Vec<&str> = labels
        ...
        .collect();
      (!ids.is_empty()).then(|| json!({ "questionId": asked.wire_id, "selectedOptionIds": ids }))
    })
    .collect();
  if picked.is_empty() {
...
async fn run_session(session: Session) {
  ...
  let setup = async {
    ...
    let (session_id, session_response) = if let Some(resume) = &request.resume {
      ...
      let new = request_draining(&client, &mut incoming, "session/new", session_params).await?;
      (
        new
          .get("sessionId")
          .and_then(Value::as_str)
    ...
    Ok::<(String, bool, Vec<SlashCommand>), HarnessError>((session_id, steer_ext, init_commands))
  };
  let (session_id, steer_ext, init_commands) = tokio::select! {
      res = setup => match res {
          ...
          Err(e) => {
              ...
              tracing::warn!(target: "helix_harness::acp", %error, "agent setup failed");
              let _ = event_tx
  ...
  // `HELIX_ACP_QUIET_SETTLE_MS` overrides; 0 disables.
  let quiet_settle: Option<Duration> = if cost_hint_enabled {
    None
  } else {
    match std::env::var("HELIX_ACP_QUIET_SETTLE_MS")
      .ok()
...
mod tests {
  use super::*;
  use helix_proto::{TodoItem, ToolCall};
  ...
  /// keyed by cursor's wire ids, not the labels helix showed the user.
  #[test]
  fn cursor_questions_round_trip_labels_back_to_option_ids() {
    ...
    // Cursor's repeatable ids never leak into helix's question ids.
  ...
  fn cursor_todo_notifications_map_to_stable_chips() {
    ...
    // Everything else is noise helix has nothing to render for.
