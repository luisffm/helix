...
async fn happy_path_maps_chunks_tools_diffs_plans_and_commands() {
  ...
  assert!(events.contains(&AgentEvent::ToolCall {
    id: "t1".into(),
    call: ToolCall::Exec {
      command: "cargo test -p helix-harness".into()
    },
  }));
  ...
  assert!(exec_output.starts_with("   Compiling helix-harness"));
...
async fn config_options_apply_requested_model_and_effort() {
  ...
  req.reasoning = Some(helix_proto::ReasoningLevel::Medium);
...
async fn ultrathink_prefixes_the_prompt_for_claude() {
  ...
  req.reasoning = Some(helix_proto::ReasoningLevel::Ultrathink);
...
/// `cargo test -p helix-harness --test acp -- --ignored real_discovery`
#[tokio::test]
...
async fn real_discovery_yields_base_models_with_traits() {
  ...
  for m in &claude {
    ...
    assert!(
      m.reasoning_levels
        .contains(&helix_proto::ReasoningLevel::Ultrathink),
...
/// `cargo test -p helix-harness --test acp -- --ignored real_claude`
#[tokio::test]
...
/// `cargo test -p helix-harness --test acp -- --ignored real_cursor`
#[tokio::test]
...
/// `cargo test -p helix-harness --test acp -- --ignored real_cursor_todos`
#[tokio::test]
  ...
  let todos: Vec<&AgentEvent> = events
    ...
    .filter(|e| {
      matches!(
        e,
        AgentEvent::ToolCall {
          call: helix_proto::ToolCall::Todo { .. },
          ..
        }
      )
    })
...
fn descriptor_surface_matches_registry_expectations() {
  ...
  assert_eq!(
    ...
    &[
      helix_proto::ReasoningLevel::Low,
      helix_proto::ReasoningLevel::Medium,
      helix_proto::ReasoningLevel::High,
    ]
  );
}
...
fn hermes_and_pi_descriptor_surfaces_match_registry_expectations() {
  ...
  assert_eq!(
    pi.reasoning_levels(),
    &[
      helix_proto::ReasoningLevel::Minimal,
      helix_proto::ReasoningLevel::Low,
      helix_proto::ReasoningLevel::Medium,
      helix_proto::ReasoningLevel::High,
      helix_proto::ReasoningLevel::XHigh,
      helix_proto::ReasoningLevel::Max,
    ]
  );
}
...
/// `cargo test -p helix-harness --test acp -- --ignored real_claude_starve`
#[tokio::test]
...
/// `cargo test -p helix-harness --test acp -- --ignored --nocapture real_all_harnesses`
#[tokio::test]
...
/// `cargo test -p helix-harness --test acp -- --ignored --nocapture real_claude_debug`
#[tokio::test]
