...
//!
//!   HELIX_AB=1 cargo test -p helix-harness --test real_quiet_ab -- --ignored --nocapture
//!
...
use helix_harness::{AcpHarness, CancellationToken, Harness, RunControls, SteerMessage};
use helix_proto::{AgentEvent, RunRequest, SandboxLevel, UserInputAnswer, UserInputQuestion};
...
fn init_env() {
  ...
  ONCE.call_once(|| {
    ...
    unsafe { std::env::set_var("HELIX_ACP_QUIET_SETTLE_MS", QUIET_MS.to_string()) };
  });
}
