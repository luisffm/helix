//! Client side: request/stream multiplexing over string frames.
...
/// A multiplexing RPC client over a string-frame duplex ([`crate::memory_client`]).
/// Cheap to clone-by-Arc internally; use one per connection.
