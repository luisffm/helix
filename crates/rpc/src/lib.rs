//! helix-rpc — the typed control plane (UiRpc / ControlRpc) over the in-process
//! transport.
//!
//! Framing: ndjson envelopes, one JSON object per WebSocket text message (or per line on
//! byte transports), matching the shape of helix's Effect RPC without the Effect runtime:
//!
...
//! `subscribe`. Both ends run over a pair of string channels ([`memory_client`]).
...
/// Spawn an in-memory server for `service` and return a connected client — the
/// in-process UI transport (ARCHITECTURE §1 "zero serialization shortcuts").
