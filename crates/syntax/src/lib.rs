//! Syntax-highlighting contracts shared by Helix's desktop surfaces.
//!
...
fn injected_languages(parent: LanguageId) -> Vec<LanguageId> {
  ...
  match parent {
    ...
    Markdown => vec![
      Rust, JavaScript, Jsx, TypeScript, Tsx, Python, Go, Json, Jsonc, Bash, Toml, Html, Css, Yaml,
      C, Cpp, CSharp, Java, Kotlin, Swift, Ruby, Php, Sql, Lua, Dockerfile, Nix, Make,
    ],
    _ => Vec::new(),
  }
}
...
fn rust_configuration() -> Result<HighlightConfiguration, HighlightError> {
  ...
  // `constant.builtin`. Helix preserves those structural roles separately.
...
fn build(value: usize) -> Widget {
  ...
  fn every_registered_grammar_loads_and_highlights_a_fixture() {
    ...
    for (language, path, source) in fixtures {
      let config = configuration(language).unwrap_or_else(|error| panic!("{language:?}: {error}"));
      assert!(
