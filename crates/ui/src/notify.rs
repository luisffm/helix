...
//!   the banner is attributed to Helix with its icon. The API is deprecated
...
//!   center — those adopt the installed Helix app's bundle identity (the
//!   terminal-notifier technique: override `-[NSBundle bundleIdentifier]` for
//!   the main bundle), so dev banners still show the Helix icon; a click may
//!   focus the installed app rather than the dev binary — acceptable for a
//!   dev-only path. Machines without Helix installed fall back to
...
//! - `HELIX_DISABLE_NOTIFICATIONS` env kill-switch + the
...
const DISABLE_ENV: &str = "HELIX_DISABLE_NOTIFICATIONS";
...
#[cfg(target_os = "macos")]
const MACOS_BUNDLE_ID: &std::ffi::CStr = c"sh.helix.app";
...
fn post_user_notification(title: &str, body: &str) -> bool {
  ...
  unsafe {
    ...
    let ns_title: *mut Object = msg_send![class!(NSString), stringWithUTF8String: title.as_ptr()];
...
mod delegate {
  ...
  pub(super) fn always_present() -> *mut Object {
    ...
    *DELEGATE.get_or_init(|| unsafe {
      let mut decl = ClassDecl::new("HelixNotifyDelegate", class!(NSObject))
        .expect("HelixNotifyDelegate registered twice");
      decl.add_method(
...
/// `-[NSBundle bundleIdentifier]` to answer the installed Helix app's id for
...
mod identity {
  ...
  extern "C" fn bundle_identifier_override(this: &Object, sel: Sel) -> *mut Object {
    unsafe {
      ...
      match ORIGINAL.load(Ordering::Relaxed) {
        0 => std::ptr::null_mut(),
        imp => {
          let original: extern "C" fn(&Object, Sel) -> *mut Object = std::mem::transmute(imp);
          original(this, sel)
        }
      }
    }
  ...
  pub(super) fn adopt_installed() -> bool {
    ...
    *ADOPTED.get_or_init(|| unsafe {
      ...
      let url: *mut Object = msg_send![workspace, URLForApplicationWithBundleIdentifier: bundle_id];
      if url.is_null() {
        return false;
      }
      ...
      let replacement: extern "C" fn(&Object, Sel) -> *mut Object = bundle_identifier_override;
...
fn post_impl(title: &str, body: &str) {
  ...
  std::thread::spawn(move || {
    ...
    let result = std::process::Command::new("notify-send")
      .args(["--app-name=Helix", "--", &title, &body])
      .stdout(std::process::Stdio::null())
      .stderr(std::process::Stdio::null())
      .status();
    match result {
