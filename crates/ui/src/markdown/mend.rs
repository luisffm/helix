...
//! `remend`, ported to helix's incremental parser).
//!
...
//! handlers don't apply here — helix renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "helix:pending-link";
...
mod tests {
  ...
  fn links_mend_to_pending_sentinel() {
    mends("[docs](https://x.dev/lo", "[docs](helix:pending-link)");
    mends("[docs](", "[docs](helix:pending-link)");
    mends("see [do", "see [do](helix:pending-link)");
    mends("![alt](https://x/i.p", "![alt](helix:pending-link)");
    stays("see [");
  ...
  fn link_urls_allow_nested_parens() {
    ...
    mends("[a](https://x.dev/(y", "[a](helix:pending-link)");
  }

  #[test]
  fn emphasis_inside_link_text_closes_inside() {
    mends("[**a", "[**a**](helix:pending-link)");
    mends("**a [b", "**a [b](helix:pending-link)**");
  }
