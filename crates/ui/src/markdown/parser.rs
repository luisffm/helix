...
fn parse_started_block(cur: &mut Cursor) -> Vec<Block> {
  ...
  match tag {
    ...
    Tag::HtmlBlock => {
      // Render raw HTML blocks as plain text (helix's markdown does the same).
...
mod tests {
  ...
  fn bare_urls_autolink() {
    assert_eq!(
      only_link("PR is updated: https://github.com/helixsh/helix/pull/31\n"),
      Some((
        "https://github.com/helixsh/helix/pull/31".into(),
        "https://github.com/helixsh/helix/pull/31".into()
      ))
    );
    assert_eq!(
