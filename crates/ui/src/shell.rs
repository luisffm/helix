//! The app shell (helix `__root.tsx`): sidebar column + main panel + optional
//! right "Changes" pane, plus the boot splash and the connection gate.
//!
//! Layout is helix's: collapsible drag-resizable sidebar (208–400px, default
...
use crate::state::{AppState, ConnectionStatus, EngineBootConfig, Indicator, format_time_ago};
...
/// left edge (helix window-controls.tsx: `left: fullscreen ? 12 : 88`). The
...
impl SettingsSection {
  ...
  /// Sidebar + header label (helix settings-sidebar.tsx SECTIONS / __root.tsx
...
/// Per-chat panel open flags (helix parity: `sessionPanels` — the terminal and
...
/// One route-history entry (helix parity: the renderer's TanStack memory
...
/// (helix window-controls.tsx semantics): every route change pushes an entry;
...
impl NavHistory {
  ...
  /// equivalent of a `replace: true` navigation (helix's boot redirect from
  ...
  /// "can go forward" (helix window-controls.tsx).
...
pub struct Shell {
  ...
  resort_epoch: usize,
  /// Dev/testing knob (`HELIX_OPEN_DIALOG`) — see [`Shell::new`].
...
impl Shell {
  pub fn new(state: Entity<AppState>, boot: EngineBootConfig, cx: &mut Context<Self>) -> Self {
    ...
    // Dev/testing knob: `HELIX_OPEN_ROUTE=settings[/<section>]` boots
  ...
  fn on_state_changed(&mut self, state: &Entity<AppState>, cx: &mut Context<Self>) {
    ...
    {
      let now = Utc::now();
      type Ping = (String, helix_proto::SessionStatus, bool, Option<String>);
      let sessions: Vec<Ping> = {
        ...
        state
          ...
          .iter()
          .map(|s| {
            use helix_proto::view::Indicator;
            let status = match helix_proto::view::effective_indicator(Some(s), now) {
              Indicator::Working => helix_proto::SessionStatus::Working,
              Indicator::AwaitingInput => helix_proto::SessionStatus::AwaitingInput,
              Indicator::Errored => helix_proto::SessionStatus::Errored,
              Indicator::None => helix_proto::SessionStatus::Idle,
            };
      ...
      // Helix window being key), so a ping for a *background chat* in a
      // focused app still stays a chime — you're already looking at
      // Helix; the sidebar dot carries the rest.
    ...
    if !self.space_boot_applied && !state.read(cx).spaces.is_empty() {
      ...
      if state.read(cx).selected_chat.is_none() {
        ...
        let target = match filter {
          ...
          None if !state.read(cx).no_project => self.settings.last_space_id.clone().filter(&exists),
          None => None,
        };
        if target.is_some() {
    ...
    if selected != self.active_chat {
      ...
      // helix's `/` route redirected into the last-used chat, leaving no
  ...
  fn add_commit_diff_surface(
    &mut self,
    commit: helix_proto::GitHistoryCommit,
    cx: &mut Context<Self>,
  ) {
  ...
  /// The flag is per chat (helix `sessionPanels`).
  fn toggle_terminal(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    if open {
      ...
      // to the prompt, no click needed (helix terminal-panel.tsx: the
      ...
      // press closes even while the terminal is focused, as in helix's
  ...
  fn on_right_pane_drag(
    ...
    // helix caps the pane at 52% of the window on top of the absolute range.
  ...
  /// reduced motion: exactly `target`. Honors `HELIX_MOTION_SCALE`.
  ...
  /// of helix __root.tsx `transition-[padding-left] duration-200 ease-out` +
  ...
  /// by every route transition (helix remounts the keyed header variants —
  ...
  fn render_title_bar(&mut self, cx: &mut Context<Self>) -> AnyElement {
    match self.route {
      ...
      Route::Settings(_) => {
        ...
        self
          .titlebar_drag_region("settings-header-titlebar", bar, cx)
          .into_any_element()
      }
    }
  }
  ...
  /// pattern (helix's `.drag` region): mark it a [`WindowControlArea::Drag`]
  ...
  /// helix window-controls.tsx): rendered once, in a paint-only overlay layer
  ...
  /// Native Windows caption controls integrated into Helix's unified
  ...
  /// Settings-mode sidebar (helix settings-sidebar.tsx): window-control
  ...
  fn render_settings_nav(
    ...
    div()
            ...
            // Back pinned to the bottom (helix settings-sidebar.tsx).
            .child(
                div().px(px(Theme::SPACE_SM)).pb(px(12.0)).child(
                    div()
                        ...
                        .child(
                            // AltArrowLeft chevron (helix settings-sidebar.tsx),
  ...
  /// One session row (helix session-row.tsx): status rail on the left
  ...
  fn render_chat_row(
    ...
    let status_label: Option<&'static str> = match status {
      helix_proto::ChatIndicator::Working => Some("Working"),
      helix_proto::ChatIndicator::AwaitingInput => Some("Input"),
      helix_proto::ChatIndicator::Errored => Some("Failed"),
      helix_proto::ChatIndicator::Completed => Some("Done"),
      helix_proto::ChatIndicator::Idle => None,
    };
    let corner_body: AnyElement = if corner_hovered {
      ...
      match status_label {
        Some(label) => {
          ...
          let glyph: AnyElement = if status == helix_proto::ChatIndicator::Completed {
            icon(icons::CHECK)
              .size(px(11.0))
              .flex_none()
    ...
    // Hover fades over transition-colors (helix session-row.tsx) — both
      ...
      let menu = popover::popover_card(&theme)
        ...
        .child(
          popover::menu_row(&theme, false, format!("chat-menu-rename-{chat_id}"))
            .id("chat-menu-rename")
            .on_click(
              cx.listener(move |this, _, _, cx| this.open_rename_chat(rename_id.clone(), cx)),
            )
        ...
        .child(
          popover::menu_row(&theme, false, format!("chat-menu-archive-{chat_id}"))
            ...
            .on_click(cx.listener(move |this, _, _, cx| this.archive_chat(archive_id.clone(), cx)))
            .child(
    ...
    if let Some(chat_id) = self.delete_confirm.clone() {
      ...
      let card = popover::dialog_card(&theme)
        ...
        .child(div().mt(px(6.0)).child(popover::dialog_body(
          &theme,
          format!(
            "\u{201C}{title}\u{201D} will be permanently deleted. This can\u{2019}t be undone."
          ),
        )))
        .child(
          div()
            ...
            .child(
              popover::btn_danger(&theme, "Delete")
                ...
                .on_click(cx.listener(move |this, _, _, cx| this.delete_chat(chat_id.clone(), cx))),
            ),
        )
        .into_any_element();
  ...
  fn render_main(&mut self, cx: &mut Context<Self>) -> AnyElement {
    ...
    let outlet: AnyElement = if has_selection {
      ...
      // New-chat canvas (helix index.tsx): the helix mark over the
  ...
  fn render_status_strip(&mut self, cx: &mut Context<Self>) -> AnyElement {
    ...
    // inner gutter (helix's `mx-auto h-6 max-w-3xl px-2`).
    let strip = div()
    ...
    for (ix, (surface, title)) in rows.into_iter().enumerate() {
      ...
      let wrapped: AnyElement = match drag {
        Some((from, over, epoch, prev_over)) if ix != from => {
          ...
          let start = crate::terminal::panel::slide_offset(ix, from, prev_over) * CHIP_SLOT;
          div()
            .relative()
  ...
  /// Backend unreachable: quiet centered copy (helix Gate `Failed`), plus a
  /// Retry affordance (the native engine doesn't self-redial).
    ...
    div()
      ...
      .child(
        div()
                    ...
                    .items_center()
                    .justify_center()
                    // Keyed per phase (helix App.tsx `<div key={phase}
...
/// The sign-in gate's faint grid backdrop (helix styles.css `.bg-grid`):
...
/// A size-6 icon button for the titlebar strip (helix window-controls.tsx:
...
fn window_control_button(
  ...
  div()
        ...
        .rounded(px(6.0))
        .cursor_pointer()
        // helix window-controls.tsx: `transition-colors` — the wash fades.
        .bg(motion::hover_blend(
            &fade_key,
...
/// A titlebar history button (helix window-controls.tsx): enabled it is a
...
/// A size-7 icon button for the main-panel header (helix __root.tsx:
...
fn header_icon_button(
  ...
  div()
        ...
        .rounded(px(6.0))
        .cursor_pointer()
        // helix __root.tsx header buttons: `transition-colors`.
        .bg(motion::hover_blend(
            &fade_key,
            crate::theme::wash(0.0),
            crate::theme::wash(0.11),
...
impl Render for Shell {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    let theme = Theme::of(cx);
    // The shell tone (helix `.frost`): the surface the sidebar sits on and
    ...
    let root = div()
            ...
            // no-ops (helix __root.tsx gates the hotkey on `!isSettings`, and
    ...
    let root = match &connection {
      ConnectionStatus::Ready => {
        ...
        // Capture knob: `HELIX_OPEN_DIALOG=model` pops the combined
        ...
        let main_width = (viewport - self.sidebar_target() - self.right_target(cx) - 10.0).max(0.0);
        ...
        // never renders it (helix __root.tsx `!isSettings && activeChat`
        ...
        let card: AnyElement = div()
          ...
          .child(main)
          .into_any_element();
        // The whole app page is one keyed `animate-in` entrance (helix
      ...
      ConnectionStatus::Connecting => root, // splash overlay covers boot
    ...
    let root = if matches!(connection, ConnectionStatus::Ready) || !cfg!(target_os = "windows") {
      root
    } else {
      root.child(
        div()
...
mod tests {
  ...
  #[test]
  fn titlebar_cluster_matches_helix_window_controls() {
    // helix window-controls.tsx: `left: fullscreen ? 12 : 88` — the
  ...
  // ---- per-session panel flags (§1.10/1.11 parity: helix sessionPanels) ----
  ...
  fn nav_push_truncates_the_forward_branch() {
    ...
    // semantics — helix's memory history PUSH truncates entries ahead).
    let mut nav = NavHistory::new(chat("a"));
    nav.push(chat("b"));
    nav.push(chat("c"));
    nav.back();
