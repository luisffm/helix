...
use gpui::{
  ...
  ListScrollEvent, ListState, ObjectFit, SharedString, StyledImage as _, StyledText, Subscription,
  Task, TextRun, Window, canvas, div, img, list, prelude::*, px, quad,
};

use helix_doc::{MessagePart, MessageRole, MessageStatus, SessionMessageEntry};
use helix_proto::ToolCall;
...
use helix_syntax::LanguageId as Lang;
...
pub enum ToolDetail {
  ...
  Stats {
    stats: Arc<Vec<helix_doc::ToolDiffStat>>,
  },
}
...
pub fn tool_detail(
  output: Option<&str>,
  diff: Option<&helix_proto::ToolDiff>,
  diff_stats: Option<&[helix_doc::ToolDiffStat]>,
) -> Option<ToolDetail> {
...
/// Reduce an inline [`helix_proto::ToolDiff`] to the changes pane's
...
pub fn diff_to_file(diff: &helix_proto::ToolDiff) -> crate::changes::FileDiff {
...
pub struct Row {
  ...
  /// its timestamp strip (helix chat-view.tsx `group`/`group-hover`).
...
/// `HELIX_FRAME_STATS=1` logs live-row render-cost percentiles (p50/p95 µs
...
fn frame_stats_enabled() -> bool {
  ...
  *ENABLED
    .get_or_init(|| std::env::var("HELIX_FRAME_STATS").is_ok_and(|v| !v.is_empty() && v != "0"))
}

const FRAME_STATS_WINDOW: usize = 240;

/// `HELIX_NO_RENDER_CACHE=1` bypasses the cross-frame flatten cache — the
/// A/B knob for the frame-cost measurement above.
fn render_cache_disabled() -> bool {
  static DISABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
  *DISABLED
    .get_or_init(|| std::env::var("HELIX_NO_RENDER_CACHE").is_ok_and(|v| !v.is_empty() && v != "0"))
}
...
///
/// The rule lives in `helix_proto::view` so the terminal viewport reports the
...
pub fn tool_group_summary(tools: &[ToolItem]) -> String {
  ...
  helix_proto::view::tool_group_summary(&pairs)
}

// `single_line` and the per-kind chip label/detail are shared with the terminal
// viewport (`helix_proto::view`): a tool must be named identically on every
...
// cell grid).
pub use helix_proto::view::{single_line, tool_chip_content};
...
struct HighlightEntry {
  ...
  document: Option<Weak<helix_syntax::HighlightedDocument>>,
  _task: Option<Task<()>>,
}
...
impl HighlightStore {
  ...
  fn request(
  ...
  ) -> Option<Arc<helix_syntax::HighlightedDocument>> {
    ...
    let task = cx.spawn(async move |this, cx| {
      ...
      let document = cx
        .background_executor()
        .spawn(async move {
          helix_syntax::highlight(helix_syntax::HighlightRequest {
            source: &code,
            path: None,
...
pub struct Transcript {
  ...
  /// strip (helix chat-view.tsx `group-hover`; the rows report hover
...
impl Transcript {
  ...
  fn step_own_turn(&mut self, cx: &mut Context<Self>) {
    ...
    if let (Some(anchor_bounds), Some(last_bounds)) = (
      ...
      let turn_height =
        f32::from(last_bounds.bottom()) - f32::from(anchor_bounds.top()) - current - base_pad;
    ...
    if motion::reduced_motion(cx) {
      ...
      self.own_turn_last_tick = None;
    } else if anchored && err <= OWN_SEND_GLIDE_SNAP_PX && err >= -(OWN_SEND_SCROLL_SLACK_PX + 2.0)
    {
  ...
  /// device (uploads targeted it) plus this device (helix's
  ...
  fn render_row(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
    ...
    let inner: AnyElement = match &row.kind {
      ...
      RowKind::InputChip { header, resolved } => input_chip(header.clone(), *resolved, &theme),
      RowKind::ErrorChip { message } => error_chip(message.clone(), &theme),
    };

    // Hover-revealed timestamp strip (helix chat-view.tsx `Timestamp`):
    ...
    div()
            ...
            .pt(px(top_gap))
            .pb(px(bottom_pad))
            // Wide gutters (helix `px-4 @3xl:px-12`) around the 46rem column.
            .px(px(48.0))
            .child(
                div()
                    .w_full()
  ...
  fn code_highlight_for(
    ...
    only: Option<usize>,
    cx: &mut Context<Self>,
  ) -> HashMap<usize, Option<Arc<helix_syntax::HighlightedDocument>>> {
    ...
    for (ix, top) in tree.blocks.iter().enumerate() {
      ...
      if let Block::CodeBlock { language, code } = &top.block
        && let Some(lang) = language
          .as_deref()
          .and_then(helix_syntax::language_for_alias)
      {
        out.insert(
          ix,
  ...
  fn tool_diff_highlight_for(
    ...
    let old = match old_text {
      Some(source) => {
        ...
        let lang = helix_syntax::language_for_path(path)?;
        Some(
          self
            .highlights
    ...
    let new = match new_text {
      Some(source) => {
        let lang = helix_syntax::language_for_path(&file.path)?;
  ...
  fn render_tool_group(
    ...
    let details: Vec<Option<Arc<ToolDetail>>> =
      tools.iter().map(|tool| tool.detail.clone()).collect();
    ...
    // Header (helix tool-group.tsx): a small chevron tile centered over the
    ...
    let header = div()
            ...
            // chips (destructive tint, helix tool-chip.tsx) and in the
...
/// The transcript ErrorChip — an exact port of helix chat-view.tsx
...
/// A small glyph standing in for the tool's icon (helix uses an icon set; a
/// quiet monochrome character keeps the tile without shipping SVGs).
/// The glyph for a tool call (helix tool-chip.tsx `toolIcon`, Solar set).
...
mod tests {
  ...
  fn live_row_parse_work_is_bounded_per_commit() {
    ...
    for i in 0..commits {
      ...
      let (tree, outcome) = parse_for_row(true, "e1#p1", &text, &mut live_parsers, &mut tree_cache);
  ...
  fn user_rows_project_file_mentions_into_chips() {
    let raw = "look at [composer.rs](helix-file:crates/ui/src/composer.rs) please";
  ...
  fn tool_diff_builds_real_hunks_with_context_and_numbers() {
    ...
    let created = helix_proto::ToolDiff {
      path: "/w/new.txt".into(),
      old_text: None,
      new_text: "only\n".into(),
    };
  ...
  fn call_block_carries_the_full_invocation() {
    ...
    let Some(ToolDetail::Output { lines, .. }) = call_block(&ToolCall::Mcp {
      server: "gh".into(),
      tool: "issues".into(),
      input: Some(serde_json::json!({"repo": "helix"})),
    }) else {
      panic!("expected an output block")
    };
    assert_eq!(lines[0].as_ref(), "gh · issues");
    assert!(lines.iter().any(|l| l.contains("\"repo\": \"helix\"")));
