...
use crate::motion::{self, HELIX_PULSE};
...
/// The floating-menu surface (helix `.glass-surface` + `menuSurface`):
...
/// One menu row (helix `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (helix's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (helix
...
/// Hairline divider between menu sections (helix `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (helix
...
/// A bordered trailing menu section (helix picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (helix dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (helix dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (helix:
