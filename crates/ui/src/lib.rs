//! helix-ui — the gpui viewport. Shell, sidebar, conversation, composer, terminal,
//! diff pane.
//!
...
//! - [`motion`] — the helix animation catalog over gpui `Animation` + cubic-bezier;
...
//! - [`loaders`] — helix pulse loader, gradient spinner, boot splash.
...
fn open_main_window(state: gpui::Entity<state::AppState>, boot: EngineBootConfig, cx: &mut App) {
  // helix window geometry: 1320×880, min 900×600 (feature-inventory §1.1).
