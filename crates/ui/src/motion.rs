//! Animation kit — the helix motion catalog as reusable helpers over gpui
...
//! - `helix-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// helix's signature entrance curve — CSS `cubic-bezier(0.16, 1, 0.3, 1)`.
...
impl MotionSpec {
  ...
  pub fn progress(&self, raw_delta: f32) -> f32 {
    ...
    let t = (raw_delta.clamp(0.0, 1.0) * total - self.delay_ms as f32) / self.duration_ms as f32;
...
/// (`transition-colors` et al. carry it unless overridden; helix never does).
...
/// Helix loader pulse period: 2.4s.
pub const HELIX_PULSE: MotionSpec = MotionSpec::new(2400, EASE);
...
/// (helix also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
/// Helix-pulse floor opacity.
// The loader constants and math live in `helix_proto::motion` (pure phase
...
// the pointer enters. The original helix puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`HELIX_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `HELIX_MOTION_SCALE=10` slows the
...
pub fn speed_scale() -> f32 {
  ...
  *SCALE.get_or_init(|| {
    std::env::var("HELIX_MOTION_SCALE")
      .ok()
...
mod tests {
  ...
  #[test]
  fn catalog_timings_match_helix() {
    ...
    assert_eq!(HELIX_PULSE.duration_ms, 2400);
