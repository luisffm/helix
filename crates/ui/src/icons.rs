...
//!
//! The set mirrors the original helix's icon usage exactly:
...
//!   are ports of the hand-drawn inline SVGs in helix's `terminal-panel.tsx` /
//!   `composer-actions.tsx`.
//! - The harness brand marks (`claude-mark`, `openai-mark`, `cursor-mark`) are
//!   ports of helix's `icons.tsx`. gpui tints SVGs with the text color, so the
...
icon_assets![
  ...
  // Mirrored variant (helix window-controls.tsx `-scale-x-100`): the LEFT
  ...
  // Hand-drawn helix glyphs (terminal-panel.tsx / composer-actions.tsx /
...
/// The Claude mark's brand orange (`#D97757`) — helix keeps it even on the
/// monochrome surface.
