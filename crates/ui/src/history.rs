...
use helix_proto::{GitHistoryCommit, GitHistoryPage, GitHistoryRef, GitHistoryRefKind};
...
impl Render for GitHistoryFetchButton {
  fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    div()
      ...
      .child(if fetching {
        crate::loaders::mini_gradient_spinner("history-fetch-all-spinner", 1.75, cx.entity_id(), cx)
          .into_any_element()
      } else {
        ...
        .update(cx, |history, cx| {
          ...
          match result.and_then(|value| {
            serde_json::from_value::<GitHistoryPage>(value)
              .map_err(|error| helix_rpc::RpcError::Failed(error.to_string()))
          }) {
            Ok(page) => {
              let old_commit_count = history.commits.len();
              let old_item_count = old_commit_count + usize::from(history.next_cursor.is_some());
              if reset {
      ...
      .when(row.is_head, |element| {
        element.child(
          div()
            ...
            .top(px(
              HISTORY_ROW_HEIGHT / 2.0 - HISTORY_NODE_RADIUS - HISTORY_HEAD_RING_PADDING,
            ))
    ...
    div()
      ...
      .children(
        refs
          ...
          .take(visible_count)
          .enumerate()
          .map(|(ref_index, reference)| Self::render_ref(reference, row_index, ref_index, theme)),
      )
