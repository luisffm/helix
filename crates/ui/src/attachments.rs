...
//!
//! Ports of helix's `composer/use-attachments.ts` (staging/upload),
...
/// Base64 chars per `UploadChunk` (helix state.ts `UPLOAD_CHUNK` — sized for
...
/// `(body_end, refs_start)` byte offsets — the tolerant equivalent of helix's
/// `ATTACHED_IMAGES_RE`.
...
/// the same stuck-offset guard as helix's `readAttachmentImage`).
...
fn protected() -> &'static Mutex<std::collections::HashSet<(String, String)>> {
  static PROTECTED: OnceLock<Mutex<std::collections::HashSet<(String, String)>>> = OnceLock::new();
