...
//!
//! helix never called `cx.set_menus`, so on macOS `NSApp.mainMenu` stayed nil:
...
pub fn init(cx: &mut App) {
  ...
  // Window verbs route to the active window. helix is single-window, so a
...
/// ⌘Q / "Quit Helix". `cx.quit()` runs the platform's standard quit routine,
...
/// The helix menu bar. macOS renders this natively; mac-only entries are gated
...
pub fn app_menus() -> Vec<Menu> {
  ...
  let mut app_items = vec![
    ...
    MenuItem::action("About Helix", About).disabled(true),
    MenuItem::separator(),
  ];
  if macos {
    app_items.extend([
  ...
  app_items.push(MenuItem::action("Quit Helix", Quit));

  let mut menus = vec![
    Menu::new("Helix").items(app_items),
