...
use helix_proto::{Chat, CheckoutDiff, GitHistoryCommit};
...
pub struct DiffHighlights {
  pub old: Option<Arc<helix_syntax::HighlightedDocument>>,
  pub new: Option<Arc<helix_syntax::HighlightedDocument>>,
}
...
impl DiffHighlights {
  ...
  pub fn spans(&self, line: &DiffLine) -> &[helix_syntax::HighlightSpan] {
...
fn excerpt_side(
  ...
  for hunk in &file.hunks {
    ...
    let source = visible
      ...
      .collect::<Vec<_>>()
      .join("\n");
    let document = helix_syntax::highlight(helix_syntax::HighlightRequest {
      source: &source,
      path: Some(path),
      fence_tag: None,
    })
  ...
  Some(Arc::new(helix_syntax::HighlightedDocument {
    language,
    lines,
  }))
}
...
fn excerpt_highlights(file: &FileDiff, language: Lang) -> Option<DiffHighlights> {
  if !helix_syntax::supports_language(language) {
    return None;
  }
...
fn sources_match_patch(file: &FileDiff, response: &helix_proto::CheckoutFileDiffText) -> bool {
  let old = response
    .old_text
    .as_deref()
...
fn full_highlights(
  file: &FileDiff,
  language: Lang,
  response: &helix_proto::CheckoutFileDiffText,
) -> Option<DiffHighlights> {
  if response.stale || response.binary || response.truncated || !sources_match_patch(file, response)
  {
    return None;
  }
  ...
  if old.is_none() && new.is_none() && helix_syntax::supports_language(language) {
    return None;
  }
  Some(DiffHighlights { old, new })
}
...
impl Changes {
      ...
      loop {
        ...
        match subscribed {
          ...
          Err(err) => {
            if this
              .update(cx, |changes, cx| {
                changes.error = Some(format!("Diff watch unavailable: {err}").into());
                cx.notify();
              })
              .is_err()
            {
    ...
    self.branches_task = Some(cx.spawn(async move |this, cx| {
      ...
      this
        .update(cx, |changes, cx| {
          ...
          match result {
            Ok(value) => {
              changes.branches = serde_json::from_value::<Vec<String>>(value).unwrap_or_default();
              let keep = changes
                .base_ref
                .as_ref()
              ...
              if !keep {
                ...
                changes.base_ref = default_base_ref(&changes.branches, current.as_deref());
              }
              changes.sync(cx);
            }
            Err(err) => {
        ...
        .update(cx, |changes, cx| {
          ...
          match result.and_then(|value| {
            serde_json::from_value::<CheckoutDiff>(value)
              .map_err(|e| helix_rpc::RpcError::Failed(e.to_string()))
          }) {
            Ok(diff) => {
  ...
  fn ensure_fold_settle(&mut self, cx: &mut Context<Self>) {
    ...
    self.fold_settle = Some(cx.spawn(async move |this, cx| {
      ...
      this
        .update(cx, |changes, _| changes.fold_settle = None)
        .ok();
    }));
  }
  ...
  fn open_ref_menu(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    let search = cx.new(|cx| ComposerInput::with_context("Search branches…", "PaletteSearch", cx));
  ...
  fn request_highlight(
    ...
    if !helix_syntax::supports_language(lang) {
    ...
    let fetch_task = match (active, engine) {
      (Some(diff), Some(engine)) => Some(cx.spawn(async move |this, cx| {
        let request = helix_proto::GetCheckoutFileDiffTextRequest {
        ...
        let response = engine
          ...
          .ok()
          .and_then(|value| {
            serde_json::from_value::<helix_proto::CheckoutFileDiffText>(value).ok()
          });
        let highlights = match response {
          Some(response) => {
            cx.background_executor()
              .spawn(async move { full_highlights(&fetch_file, lang, &response).map(Arc::new) })
              .await
          }
          None => None,
        };
  ...
  fn render_file_header(
    ...
    // Chevron (helix checkout-diff-sidebar): chevron-right closed,
  ...
  pub fn render_header_controls(&mut self, cx: &mut Context<Self>) -> AnyElement {
    ...
    if let Some(commit) = self.commit.clone() {
      ...
      return div()
        ...
        .child(
          Self::header_button("changes-fold-all", crate::icons::FOLD_VERTICAL, &theme).on_click(
    ...
    let history_fetch_button = (scope == DiffScope::History).then(|| self.history_fetch_button(cx));
    let trigger = div()
...
fn diff_line_row(
  line: &DiffLine,
  spans: &[helix_syntax::HighlightSpan],
  theme: &Theme,
  gutter_px: f32,
) -> AnyElement {
...
/// column, and paint-only syntax runs (helix checkout-diff-sidebar).
...
";
  ...
  fn phases() {
    ...
    summarized.files.push(helix_proto::DiffFileSummary {
      path: "x".into(),
      old_path: None,
  ...
  fn full_diff_highlights_map_old_new_and_context_by_source_line() {
    ...
    assert!(
      highlights
        .spans(&deleted)
        .iter()
        .any(|span| span.kind == helix_syntax::HighlightKind::Function)
    );
    assert!(
      highlights
        .spans(&added)
  ...
  fn excerpt_parses_old_and_new_hunks_as_separate_documents() {
    ...
    assert!(
      highlights
        .spans(deleted)
        .iter()
        .any(|span| span.kind == helix_syntax::HighlightKind::Comment)
    );
    assert!(
      highlights
        .spans(added)
  ...
  fn mismatched_full_sources_are_rejected_atomically() {
    ...
    let response = helix_proto::CheckoutFileDiffText {
