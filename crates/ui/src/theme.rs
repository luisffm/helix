...
//! lightness steps; the same scale helix's Tailwind theme used) into gpui [`Hsla`].
...
use helix_syntax::HighlightKind;
...
impl Theme {
  ...
  /// Main-panel header height (helix `h-11`) — in-card headers (changes pane).
  ...
  /// Reserved status strip under the content outlet (helix `h-6`) — the
  ...
  /// (see [`crate::frost::frosted`]). Dark: the reference helix
...
mod tests {
  ...
  fn neutral_950_is_0a0a0a() {
    // oklch(0.145 0 0) is Tailwind neutral-950, helix's app background.
