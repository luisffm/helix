//! Loaders: the helix pulse loader, the gradient matrix spinner, and the boot
...
use crate::motion::{self, GRADIENT_SPIN, HELIX_PULSE, PULSE_STAGGER, SPLASH_OUT};
use crate::theme::Theme;

// Shared with the terminal viewport (`helix_proto::motion`) so both animate the
// same loaders from the same numbers.
pub use helix_proto::motion::{
  HELIX_CELLS, MARK_CELLS, MARK_SPREAD, MATRIX_SIDE, mark_cell_stagger,
};

/// The animated helix mark (helix-loader.tsx `HelixLoader`): the full logo
...
pub fn helix_mark_loader(
  ...
  let delta = motion::pulse_delta(&HELIX_PULSE, view, cx);
  div()
    .relative()
    .w(px(820.0 * scale))
    .h(px(height_px))
    .children(MARK_CELLS.iter().map(move |&(x, y)| {
      ...
      div()
        ...
        .child({
          ...
          // the stagger ADDS phase (helix-loader.tsx delayFor).
...
/// The helix wave loader: a row of cells pulsing opacity 0.08→1 / scale 0.9→1
...
pub fn helix_loader(
  ...
  div()
    ...
    .items_center()
    .gap(px(slot / 2.0))
    .children((0..HELIX_CELLS).map(move |i| {
...
pub use helix_proto::motion::{GSPIN_DIM, GSPIN_ROW_TINTS};

/// The gradient matrix spinner (WorkingIndicator), ported from helix's
...
/// Full-window boot splash (helix App.tsx `Splash`): the animated helix mark
...
/// The landing page's hero helix, monochrome (user request: white instead of
...
/// The landing page's hero helix (apps/landing/public/index.html
...
const _: () = {
  ...
  assert!(HELIX_PULSE.duration_ms == 2400);
