...
use gpui::{
  ...
  ScrollWheelEvent, SharedString, Style, StyledImage as _, Subscription, Task, TextRun, TextStyle,
  UTF16Selection, UnderlineStyle, Window, WrappedLine, actions, div, fill, img, point, prelude::*,
  px, quad, relative, size,
};
use unicode_segmentation::UnicodeSegmentation;

use helix_doc::{MessagePart, MessageRole, SessionCommandPayload, SessionMessageEntry};
use helix_proto::{
  FileSearchMatch, HarnessId, RunRequest, SandboxLevel, SlashCommand, UserInputAnswer,
  UserInputQuestion,
};
use helix_rpc::{RpcError, methods};
...
/// Expanded-mode textarea vertical padding: `pt-4 pb-1` (helix composer.tsx
/// line 578) = 16 + 4.
...
/// 260)` (helix composer.tsx line 235). The 76px floor applies even when
...
/// children; composer/styles.tsx pickerChip) + `pb-2.5` (10) — helix
...
/// Staged-attachment strip metrics (helix attachment-ui.tsx AttachmentStrip:
...
const FILE_MENTION_SCHEME: &str = "helix-file:";
...
impl ComposerInput {
  ...
  pub fn set_mention_controls(&mut self, open: bool, has_selection: bool, cx: &mut Context<Self>) {
  ...
  pub fn replace_mention(
    ...
    self.content = self.content[..range.start].to_owned() + &inserted + &self.content[range.end..];
    self.refresh_projection();
    let cursor = range.start + inserted.len() + existing_separator.map(char::len_utf8).unwrap_or(0);
      ...
      this
        .update(cx, |input, cx| {
          let live = input
            .mention_tooltip
            .target()
            .is_some_and(|target| input.mention_hits.iter().any(|hit| &hit.target == target));
          ...
          if next != input.mention_tooltip {
            ...
            if let MentionTooltipPhase::Visible { target, generation } = &input.mention_tooltip {
  ...
  fn on_mention_pointer_move(&mut self, position: Point<Pixels>, cx: &mut Context<Self>) {
    ...
    match next {
      MentionTooltipPhase::Waiting { target, .. } => self.start_mention_tooltip_wait(target, cx),
      _ => {
        self.invalidate_mention_tooltip();
  ...
  fn bounds_for_display_range(&self, range: Range<usize>) -> Vec<Bounds<Pixels>> {
    ...
    for (line_ix, line) in self.last_lines.iter().enumerate() {
      ...
      for (row_ix, row_start, segment) in display_row_segments(local_start..local_end, row_ends) {
        let row_y = y_offset + self.line_height * row_ix;
        let start_x = if segment.start == row_start {
          px(0.0)
        } else {
          line
            .position_for_index(segment.start, self.line_height)
            .map(|point| point.x)
            .unwrap_or(px(0.0))
        };
...
pub struct Composer {
  ...
  /// `Window` — the `HELIX_ATTACH_PREVIEW` boot knob opens in `new`).
...
impl Composer {
  ...
  pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
    ...
    let input_events = cx.subscribe(&input, |this: &mut Self, _, event, cx| match event {
      ...
      ComposerInputEvent::Edited | ComposerInputEvent::CursorMoved => this.on_input_edited(cx),
    ...
    // a rig) — `HELIX_ATTACH=/path/a.png[,/path/b.png]`, and
    // `HELIX_ATTACH_PREVIEW=1` boots with the first one's lightbox open.
    if let Ok(spec) = std::env::var("HELIX_ATTACH") {
      let staged: Vec<StagedAttachment> = spec
        .split(',')
        .filter(|s| !s.trim().is_empty())
        .filter_map(
          |path| match attachments::stage_file(std::path::Path::new(path.trim())) {
            Ok(att) => Some(att),
            Err(err) => {
              tracing::warn!(%path, error = %err, "HELIX_ATTACH stage failed");
              None
            }
          },
        )
        .collect();
      if std::env::var("HELIX_ATTACH_PREVIEW").is_ok_and(|v| v == "1")
  ...
  /// Capture-knob passthrough (`HELIX_OPEN_DIALOG=model`): open the
  ...
  fn render_slash_popup(&self, theme: &Theme, cx: &mut Context<Self>) -> Option<gpui::AnyElement> {
      ...
      _ => {
        if let Some(wizard) = self.wizard.as_ref() {
          ...
          let released = input_request_resolved(&transcript, &wizard.request_id)
            || (!transcript.is_empty() && !self.answered_requests.contains(&wizard.request_id));
          if released {
            self.wizard = None;
            self.advance_task = None;
            self
                    ...
                    match &plan {
                        ...
                        crate::pickers::CheckoutPlan::NewWorktree { base } => {
                            ...
                            if let (Some(repo_path), Some(base)) = (&space_path, base) {
                                ...
                                let worktree: helix_proto::Worktree = serde_json::from_value(value)
  ...
  fn schedule_auto_advance(&mut self, cx: &mut Context<Self>) {
    self.advance_task = Some(cx.spawn(async move |this, cx| {
      cx.background_executor()
        ...
        .await;
      this
        .update(cx, |composer, cx| composer.wizard_advance(cx))
        .ok();
    }));
  }
  ...
  /// The agent-asked-a-question panel (helix question-panel.tsx), rendered in
  ...
  fn render_wizard(&mut self, cx: &mut Context<Self>) -> gpui::AnyElement {
    ...
    let options = question.options.iter().enumerate().map(|(ix, label)| {
      ...
      // (typed answers win — helix question-panel.tsx `isSel`).
      ...
      div()
                ...
                // helix question-panel.tsx option rows: `transition-colors`.
                .bg(if picked {
    ...
    div()
      ...
      .on_key_down(
        cx.listener(|this, event: &KeyDownEvent, window, cx| this.on_wizard_key(event, window, cx)),
      )
      .rounded(px(26.0))
      .border_1()
  ...
  fn render_send_button(
    ...
    let theme = Theme::of(cx);
    // Helix composer-actions.tsx: a size-7 filled circle — up-arrow to
...
impl Render for Composer {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    // Centered composer column (helix `mx-auto w-full max-w-3xl`).
    let container = div()
      .w_full()
      .max_w(px(768.0))
      .mx_auto()
      ...
      .when_some(failure, |el, message| {
        // helix composer.tsx `Notice` (matches the transcript
    ...
    let steer_queues = mode == SendButtonMode::Steer
      && self.pickers.read(cx).resolved_steering_mode(cx)
        == Some(helix_proto::SteeringMode::TurnBoundary);
    ...
    // need the full-width actions row (helix composer-actions.tsx
    // `mustExpand = isNew || …`).
    ...
    let (pill_height, morph_t, morphing) = match self.flip_morph {
      Some(m) if !m.done(now_ms) => (m.height(target_height, now_ms), m.progress(now_ms), true),
    ...
    let attach = div()
            ...
            .rounded_full()
            .cursor_pointer()
            // helix composer-actions.tsx attach: `transition-colors`.
    ...
    // The pill chrome (helix composer.tsx): `rounded-[26px] border
          ...
          div()
            ...
            .child(
              div()
                                ...
                                // helix composer-actions.tsx): identical
...
mod tests {
  ...
  fn file_mentions_serialize_to_strict_local_markdown() {
    ...
    assert_eq!(
      raw,
      "[a file#\\[x\\].rs](helix-file:src/a%20file%23%5Bx%5D.rs)"
    );
    let links = file_mention_links(&raw);
    ...
    assert_eq!(folder, "[components](helix-file:src/components/)");
  ...
  fn sent_mention_display_leaves_plain_prompts_untouched() {
    ...
    assert_eq!(
      sent_mention_display("what is a helix-file: link?"),
      None,
      "scheme substring without a valid mention link"
    );
    assert_eq!(
      sent_mention_display("[a.rs](helix-file:../a.rs)"),
      None,
  ...
  #[test]
  fn auto_grow_math() {
    // The source heights (helix composer.tsx line 235 clamp, composer-
    ...
    // Caps at a 260px textarea box (helix max-h-[260px] / the JS clamp).
    assert_eq!(
