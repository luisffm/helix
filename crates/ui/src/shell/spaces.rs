...
impl Shell {
  ...
  fn open_spaces_menu(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    let search = cx.new(|cx| ComposerInput::with_context("Search projects…", "PaletteSearch", cx));
  ...
  fn render_spaces_menu(&mut self, theme: &Theme, cx: &mut Context<Self>) -> AnyElement {
    ...
    popover::popover_card(theme)
      ...
      .track_focus(&focus)
      .on_key_down(
        cx.listener(|this, event: &gpui::KeyDownEvent, _, cx| this.spaces_menu_key(event, cx)),
      )
  ...
  fn add_space_filtered(&self, cx: &App) -> Vec<helix_proto::FolderEntry> {
  ...
  pub(super) fn render_add_space_overlay(
    ...
    // ── breadcrumbs ("MacBook Pro / Projects / helix"): the quiet mono
      ...
      let menu = popover::popover_card(&theme)
        ...
        .child(
          popover::menu_row(&theme, false, format!("space-menu-rename-{space_id}"))
            ...
            .on_click(
              cx.listener(move |this, _, _, cx| this.open_rename_space(rename_id.clone(), cx)),
            )
          ...
          div()
            ...
            .child(
              popover::btn_danger(&theme, "Remove")
                ...
                .on_click(
                  cx.listener(move |this, _, _, cx| this.delete_space(space_id.clone(), cx)),
                ),
            ),
        )
        .into_any_element();
