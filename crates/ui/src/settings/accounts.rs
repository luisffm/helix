...
//! account-shaped loading skeletons. Helix retargets devices from the settings
...
use gpui::{
  AnyElement, Context, Entity, Hsla, SharedString, Subscription, Task, Window, div, prelude::*, px,
};
...
pub fn force_usage_for(trigger: LoadTrigger) -> bool {
  match trigger {
    LoadTrigger::Mount | LoadTrigger::Retry | LoadTrigger::Refresh | LoadTrigger::PostLogin => true,
    LoadTrigger::PostAction => false,
  }
}

/// Compact absolute reset moment (helix settings.agents.tsx `formatReset`):
...
/// in the empty-state copy, helix settings.agents.tsx `PROVIDERS`).
...
impl LoginFlow {
  /// Dialog title (helix: "Add Claude account" / "Add Codex account").
...
impl AccountsPage {
      ...
      loop {
        ...
        let outcome = this.update(cx, |page, cx| {
          ...
          match result
            .as_ref()
            .ok()
            .and_then(|value| serde_json::from_value::<AgentLoginPoll>(value.clone()).ok())
          {
  ...
  /// One usage window (helix settings.agents.tsx `UsageMeter`): label ·
  /// 5px rounded-full bar (indigo → amber ≥80% → red ≥95%) · "NN% used" ·
  /// quiet reset time.
  fn render_usage_meter(
    &self,
    window: &helix_proto::AgentUsageWindow,
    theme: &Theme,
    now: DateTime<Utc>,
  ) -> AnyElement {
  ...
  /// One account row (helix settings.agents.tsx `AccountRow`): initial
  ...
  fn render_account_row(
    ...
    // Actions only on INACTIVE accounts (helix `{!account.active && …}`):
    ...
    let actions: Option<gpui::Div> = (!account.active).then(|| {
      div()
        ...
        .when(account.switchable, |el| {
          el.child(
            crate::popover::btn_primary(theme, if is_busy { "Switching…" } else { "Switch" })
              ...
              .when(is_busy, |el| el.opacity(0.5))
              .on_click(cx.listener(move |this, _, _, cx| {
                this.account_action(methods::ACTIVATE_AGENT_ACCOUNT, &switch_account, cx);
              })),
          )
        })
    });
      ...
      .child(
        div()
          ...
          .map(|el| {
            ...
            // (helix: `usage ? meters : "Usage unavailable"…`).
  ...
  fn render_login_dialog(
    ...
    let url_link = |id: &'static str, label: &'static str, url: &str, cx: &mut Context<Self>| {
      ...
      // "Reopen the …" text link (helix: `text-[12px]
                ...
                .child(
                  div()
                    ...
                    .child(
                      message
                        .clone()
                        .unwrap_or_else(|| SharedString::from("Waiting for the browser…")),
                    ),
                ),
            )
          })
  ...
  /// A ghost account row (helix settings.agents.tsx `SkeletonRow`): avatar,
  ...
  fn render_skeleton_row(
    ...
    use crate::motion;
    let delta = motion::pulse_delta(&motion::HELIX_PULSE, cx.entity_id(), cx);
...
impl Render for AccountsPage {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    // Brand mark inside a 24px centered box (helix: `grid size-6
    ...
    // One section per provider (helix settings.agents.tsx `ProviderSection`):
      ...
      Loadable::Ready(snapshot) => {
        ...
        PROVIDERS
          ...
          .map(|(harness, name, cli)| {
            ...
            // EVERY warning renders its own strip (helix maps them).
      ...
      .child(
        widgets::page_column()
                    .child(
                        div()
                            ...
                            .child(
                                ...
                                // dimmed while a refresh is in flight (helix
                                // `disabled:opacity-50`).
                                widgets::ghost_action(&theme)
                                    .id("accounts-refresh")
                                    .flex_none()
                    ...
                    .child(widgets::page_subtitle(
                        &theme,
                        "The Claude Code and Codex logins on this device. Helix detects the \
                    ...
                    .children(sections)
                    // Footer note (helix: `mt-6 text-[12px] leading-relaxed
...
mod tests {
  ...
  #[test]
  fn usage_thresholds_match_helix() {
