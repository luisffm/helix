...
//! (helix parity: localStorage `helix.composer.defaults:v1`, defaults.ts).
//!
...
use helix_proto::{HarnessId, ReasoningLevel};

const FILE_NAME: &str = "composer-defaults.json";

/// Remembered model per harness — id plus display label, mirroring helix's
...
pub struct ComposerDefaults {
  ...
  /// Last reasoning level picked (global, like helix's `reasoning` key).
...
impl ComposerDefaults {
  ...
  /// Remember a pick (helix `saveDefaults({ harness, modelByHarness })`).
  ...
  pub fn remember_labels<'a>(&mut self, models: impl Iterator<Item = (&'a str, &'a str)>) -> bool {
