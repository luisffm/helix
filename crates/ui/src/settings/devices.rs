...
/// Human platform label (helix settings.devices.tsx `platformLabel`).
  ...
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let rows: Vec<AnyElement> = devices
      ...
      .map(|(ix, device)| {
        ...
        // tone so it "cuts" the tile — helix settings.devices.tsx
        ...
        // "Added {time ago}" — always present (helix settings.devices.tsx).
        ...
        widgets::card_row(&theme, ix == 0)
          ...
          .child(
            // `opacity-70 hover:opacity-100` (helix: also rises on
