...
use gpui::{
  Context, Entity, EventEmitter, FocusHandle, KeyDownEvent, SharedString, Window, div, prelude::*,
  px,
};
...
pub struct ShortcutsPage {
  ...
  /// conflicts never persist; they're refused at record time, as in helix.
  ...
  fn on_key_down(&mut self, event: &KeyDownEvent, cx: &mut Context<Self>) {
    ...
    match record_key(
      ...
      RecordOutcome::Set(combo) => {
        ...
        // (helix settings.shortcuts.tsx: "… is already assigned to …").
...
/// One-line purpose copy per shortcut (helix lib/shortcuts.ts
...
impl Render for ShortcutsPage {
  fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    let rows = ShortcutId::ALL.into_iter().enumerate().map(|(ix, id)| {
      ...
      // helix settings.shortcuts.tsx row: min-h-[72px] px-5 gap-5, label
    ...
    // the message names the specific clash (helix settings.shortcuts.tsx).
    ...
    div()
      ...
      .on_key_down(cx.listener(|this, event: &KeyDownEvent, _, cx| this.on_key_down(event, cx)))
      .child(
        widgets::page_column()
          .child(
            div()
              ...
              .child({
                ...
                widgets::ghost_action(&theme)
                  ...
                  .when(!disabled, |el| {
                    el.hover(|s| s.bg(crate::theme::ink(0.04)).text_color(theme.text))
...
mod tests {
  ...
  fn conflicting_records_are_refused() {
    // helix parity: a combo bound elsewhere is refused at record time (the
