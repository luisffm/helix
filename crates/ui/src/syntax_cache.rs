...
use helix_syntax::{HighlightedDocument, LanguageId};
...
impl SyntaxHighlightCache {
  ...
  pub fn insert(&mut self, key: DocumentHighlightKey, document: Arc<HighlightedDocument>) -> bool {
  ...
  std::mem::size_of::<HighlightedDocument>()
    .saturating_add(
      document
        .lines
        .capacity()
        .saturating_mul(std::mem::size_of::<Vec<helix_syntax::HighlightSpan>>()),
    )
    .saturating_add(document.lines.iter().fold(0usize, |total, line| {
      total.saturating_add(
        line
          .capacity()
          .saturating_mul(std::mem::size_of::<helix_syntax::HighlightSpan>()),
      )
    }))
}
