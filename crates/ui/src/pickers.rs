...
use helix_engine::registry::HarnessDescriptor;
use helix_proto::{
...
pub enum CheckoutPlan {
  ...
  /// `CreateWorktree` off `base` on send (helix mints a `helix/<name>`
...
/// lead with the flagship — helix's `pickDefaultModel` Opus preference maps to
/// the same row here).
pub fn default_model(models: &[Model]) -> Option<&Model> {
  models.first()
}

/// A model's default reasoning: X-High when the ladder offers it (helix
...
/// or foreign level — helix use-run-config.ts's derived-model discipline).
  ...
  model.is_some_and(|model| {
    model.options.iter().any(|option| {
      selections
        .get(&option.id)
        .and_then(|v| v.as_str())
        .is_some_and(|id| id != option.default_choice && option.choices.iter().any(|c| c.id == id))
    })
  })
}
...
pub fn browser_rows(listing: &FolderListing) -> Vec<&helix_proto::FolderEntry> {
...
pub struct Pickers {
  ...
  config: DraftConfig,
  /// Sticky last-used picks (helix `helix.composer.defaults:v1`): seeds the
  ...
  focus: FocusHandle,
  /// `HELIX_OPEN_PICKER` boot: keep claiming focus until it sticks, so
...
impl Pickers {
  pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
    ...
    // Dev/testing knob: `HELIX_OPEN_PICKER=model|traits|repo|branch` boots
    // with that popover open — synthetic input can't reach the app on
    // headless compositors, so captures need a data-side path.
    let boot_open = match std::env::var("HELIX_OPEN_PICKER").ok().as_deref() {
  ...
  fn effective_harness(&self, cx: &App) -> Option<HarnessId> {
    ...
    // model (it stays available under `HELIX_HARNESS=mock`).
    self
      .harnesses
      .ready()
  ...
  pub fn resolved_steering_mode(&self, cx: &App) -> Option<helix_proto::SteeringMode> {
    let harness = self.effective_harness(cx)?;
    self
      .harnesses
      .ready()
  ...
  /// Capture knob (`HELIX_OPEN_DIALOG=model`): open the combined
    ...
    if !query.is_empty() {
      ...
      for descriptor in &descriptors {
        ...
        for model in models {
          ...
          let by_harness =
            popover::match_rank(&query, &format!("{} {}", descriptor.name, model.label))
              .map(|rank| rank + 2);
    ...
    let body: AnyElement = if rows.is_empty() {
      ...
      div()
        ...
        .children(rows.into_iter().enumerate().map(|(ix, space)| {
          ...
          popover::menu_row_nav(&theme, is_selected, ix == active, format!("space-row-{ix}"))
            .id(("space-row", ix))
            .on_click(cx.listener(move |this, _, _, cx| {
  ...
  fn trigger_chip(
    ...
    div()
            ...
            // helix composer/styles.tsx `pill`: `transition-colors` — the wash
  ...
  fn popover_frame(&self, width: f32, content: AnyElement, cx: &mut Context<Self>) -> AnyElement {
    ...
    popover::popover_card(&theme)
            .w(px(width))
            // helix caps its tallest picker at min(640px, 75vh).
            .max_h(px(640.0))
  ...
  /// picker's rail + list panes bleed to the card edge (helix
  ...
  fn popover_frame_flush(
    ...
    popover::popover_card_flush(&theme)
      ...
      .track_focus(&self.focus)
      .on_key_down(
        cx.listener(|this, event: &KeyDownEvent, window, cx| this.on_key_down(event, window, cx)),
      )
      .on_mouse_down_out(cx.listener(|this, _, _, cx| this.close(cx)))
      .flex()
      .flex_col()
  ...
  fn search_box(&self, theme: &Theme) -> AnyElement {
    popover::search_input_frame(theme, self.search.clone().into_any_element()).into_any_element()
  }
  ...
  /// The combined harness + model switcher (helix harness-model-picker.tsx):
...
pub(crate) fn normalize_model_rows(harness: HarnessId, models: Vec<Model>) -> Vec<Model> {
  ...
  let catalog = match harness {
    HarnessId::ClaudeCode => helix_harness::claude::catalog::static_models(),
    _ => Vec::new(),
  };
    ...
    .filter_map(|mut model| {
      ...
      if let Some(base) = strip_1m(&model.id.clone()) {
        ...
        if !model.options.iter().any(|o| o.id == "contextWindow") {
          model.options.push(helix_proto::ModelOption {
            id: "contextWindow".into(),
            label: "Context Window".into(),
            choices: vec![
              helix_proto::ModelOptionChoice {
                id: "200k".into(),
                label: "200K".into(),
              },
              helix_proto::ModelOptionChoice {
...
/// `HELIX_HARNESS=mock` (the e2e/dev rig) opts the mock harness into the UI;
/// production launches never set it, so the mock never surfaces there.
fn mock_harness_enabled() -> bool {
  std::env::var("HELIX_HARNESS")
    .ok()
    .as_deref()
    .map(str::trim)
    == Some("mock")
...
/// `HELIX_HARNESS=mock` shows it; otherwise it only remains when it's
...
fn offered_harnesses_impl(list: &[HarnessDescriptor], allow_mock: bool) -> Vec<HarnessDescriptor> {
  ...
  let offered: Vec<HarnessDescriptor> = visible
    .iter()
    .filter(|d| {
      helix_engine::registry::descriptor_enabled(d) || (allow_mock && d.id == HarnessId::Mock)
    })
    .cloned()
    .collect();
...
impl Render for Pickers {
  fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    let theme = Theme::of(cx).clone();
    // A HELIX_OPEN_PICKER popover never went through `toggle`, so claim
    ...
    self.prefetch_models(cx);
    // A popover opened data-side (HELIX_OPEN_PICKER) never went through
    ...
    // Chip shows the model's display name alone (helix `modelText`); the
    ...
    // attach + send after this element (helix composer-actions.tsx
    // arrangement).
    let left = div()
      .flex()
      .flex_row()
...
mod tests {
  use super::*;
  use helix_proto::{FolderEntry, Model, ModelOption, ModelOptionChoice};
  ...
  fn completion_prefix_lengths() {
    ...
    assert_eq!(completion_prefix_len("helix", "helix"), Some(5));
    assert_eq!(completion_prefix_len("helix", ""), Some(0));
    assert_eq!(completion_prefix_len("helix", "dev"), None);
  ...
  fn browser_navigation_reducer() {
    ...
    assert_eq!(browser_rows(&listing)[1].name, "helix");
  }
  ...
  fn mock_harness_hidden_unless_alone() {
    let descriptor = |id: HarnessId, name: &str| HarnessDescriptor {
      ...
      steering_mode: helix_proto::SteeringMode::StepBoundary,
      reasoning_levels: vec![],
      installed: true,
      enabled: None,
    };
    ...
    // …and opted back in by HELIX_HARNESS=mock (the e2e rig).
  ...
  fn offered_harnesses_follow_the_catalog_enabled_flags() {
    ...
    let offered = offered_harnesses_impl(&catalog(Some(false), Some(false), Some(false)), false);
