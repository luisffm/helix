...
use helix_proto::{TerminalEvent, TerminalSession};
...
impl TerminalPanel {
  ...
  fn spawn_session(
    ...
    cx.spawn(async move |this, cx| {
        ...
        Err(err) => {
          ...
          let _ = this.update(cx, |panel, cx| {
            if let Some(tab) = panel.tab_mut(&chat, key) {
              tab
                .emulator
                .feed(format!("\x1b[31mfailed to open terminal: {err}\x1b[0m\r\n").as_bytes());
              tab.exited = Some(-1);
              cx.notify();
            }
          });
      ...
      loop {
        let Ok(after_seq) =
          this.update(cx, |panel, _| panel.tab_mut(&chat, key).map(|t| t.last_seq))
        else {
  ...
  fn on_key_down(&mut self, event: &KeyDownEvent, _window: &mut Window, cx: &mut Context<Self>) {
    ...
    if ks.key == "c" && (mods.platform || (mods.control && mods.shift)) && self.copy_selection(cx) {
      cx.stop_propagation();
      return;
    }
    let app_cursor = self
  ...
  fn with_active_emulator<R>(&mut self, cx: &App, f: impl FnOnce(&mut Emulator) -> R) -> Option<R> {
    let chat = self.selected_chat(cx)?;
  ...
  fn render_tab_bar(&mut self, chat: &str, cx: &mut Context<Self>) -> impl IntoElement + use<> {
    ...
    // Helix terminal-panel.tsx: `flex h-10 items-center border-b
    ...
    div()
            ...
            .children(
                tab_elements
                    ...
                    .map(|(ix, key, title, selected, exited)| {
                        ...
                        // Helix tab: `h-7 rounded-lg pl-2 pr-1 gap-1.5 text-xs`,
                        ...
                        let tab_el = div()
                            ...
                            .pr(px(4.0))
                            .rounded(px(8.0))
                            // helix terminal-panel.tsx tab: `transition-colors`.
            ...
            .child(
                div()
                    ...
                    .rounded(px(8.0))
                    .cursor_pointer()
                    // helix terminal-panel.tsx icon buttons: `transition-colors`.
                    .bg(motion::hover_blend(
                        "term-new-tab",
                        gpui::transparent_black(),
            ...
            // Collapse chevron pinned right (helix "Hide terminal" ⌘J).
            .child(div().flex_1())
            .child(
                div()
