...
/// Session-row sub-line, "project · branch" (helix `chatLocation`): the repo
...
/// Per-kind chip label + one-line detail. Labels match helix's `describeTool`
...
fn tool_chip_content_raw(call: &crate::ToolCall) -> (&'static str, String) {
  ...
  match call {
    ...
    ToolCall::ApplyPatch { path } => ("Patch", path.clone().unwrap_or_else(|| "workspace".into())),
...
pub fn tool_group_summary(tools: &[(crate::ToolCall, bool)]) -> String {
  ...
  for (call, is_error) in tools {
    ...
    match call {
      ...
      ToolCall::Search { .. } | ToolCall::Glob { .. } | ToolCall::WebSearch { .. } => searches += 1,
  ...
  // Capitalize the first segment only (helix's style).
...
/// read the same on every surface. `helix-ui` has the oklch→sRGB math.
pub mod dot {
...
pub enum CheckoutPlan {
  ...
  /// `CreateWorktree` off `base` on send (the engine mints a `helix/<name>`
