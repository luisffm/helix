...
//!
//! In helix these were synced Postgres rows; in helix they live in the per-org
...
pub struct Device {
  ...
  /// First registration time (helix devices.created_at — the Devices page
...
pub struct Chat {
  ...
  /// continuity across engine restarts (helix's `chats.harness_session_id`).
...
pub struct Worktree {
  ...
  pub path: String,
  pub branch: String,
  /// Generated worktree folder name (`helix/<name>` is its branch).
  #[serde(default)]
  pub name: String,
