...
pub struct RunRequest {
  ...
  /// refs (helix's `withAttachments` transport — that's what persists in the
...
///
/// Mirrors helix's `AgentEvent` tagged enum.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
mod tests {
  ...
  fn run_request_attachments_default_and_round_trip() {
    ...
    let round: RunRequest = serde_json::from_value(serde_json::to_value(&req).unwrap()).unwrap();
