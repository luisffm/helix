//! Loader motion — the pure math behind helix's loading indicators.
//!
//! These are the curves and constants the gpui viewport animates with
//! (`helix-ui/src/motion.rs`, `helix-ui/src/loaders.rs`), lifted here so any
...
/// Helix loader pulse period.
pub const HELIX_PULSE_MS: u64 = 2_400;
/// Gradient matrix spinner wave period.
pub const GRADIENT_SPIN_MS: u64 = 750;

/// Cells in the helix wave loader.
pub const HELIX_CELLS: usize = 5;
/// Side length of the gradient spinner matrix.
pub const MATRIX_SIDE: usize = 3;

/// Helix loader cells rest at this opacity between pulses.
...
/// Per-row tints of the gradient matrix spinner — helix's "sunrise" gradient
...
/// Helix loader cell opacity for a phase: 0.08 → 1 → 0.08.
pub fn pulse_opacity(phase: f32) -> f32 {
  PULSE_MIN_OPACITY + (1.0 - PULSE_MIN_OPACITY) * pulse_wave(phase)
}

/// Helix loader cell scale for a phase: 0.9 → 1 → 0.9.
...
/// ported from helix's `gradient-spin-pulse` keyframes: full at the cycle
...
/// The helix mark's pixels — `[x, y]` of each 100×100 cell on the 820×940
/// canvas (helix's `logo.tsx` CELLS), shared by the static mark and the
/// animated loader.
#[rustfmt::skip]
...
/// Per-cell stagger along the helix's flight axis. The stagger *adds* phase
